...
namespace Baci.ArcGIS.Geoprocessor.RasterAnalysisTools
{
	...
	public class ZonalStatisticsAsTable : AbstractGPProcess
	{
		...
		/// <para>输出表的名称。</para>
		/// <para>如果表已存在，则系统将提示您提供其他名称。</para>
		/// </param>
		...
		/// <para>所有—将为整型值栅格计算所有统计数据。将为浮点型值栅格计算所有统计数据（中值和百分位数除外）。这是默认设置。</para>
		...
		/// <para>Range—计算待汇总栅格图层中与输出像元同属一个区域的所有像元的最大值与最小值之差。</para>
		...
		/// <para>最近—将使用最接近所需的百分位数的可用值。</para>
