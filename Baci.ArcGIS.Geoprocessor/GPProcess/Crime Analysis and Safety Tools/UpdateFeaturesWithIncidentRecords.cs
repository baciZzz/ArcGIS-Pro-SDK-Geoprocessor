...
namespace Baci.ArcGIS.Geoprocessor.CrimeAnalysisandSafetyTools
{
	...
	/// <para>根据 x,y 坐标或街道地址更新现有表或将非空间表转换为点要素，并使用表中的新记录或更新记录信息更新现有数据集。</para>
	...
	public class UpdateFeaturesWithIncidentRecords : AbstractGPProcess
	{
		...
		/// <para>非空间表或表包含定义记录位置的 x 和 y 坐标或地址。</para>
		/// </param>
		/// <param name="TargetFeatures">
		/// <para>Target Features</para>
		/// <para>要更新的点要素类、点要素图层或表。</para>
		/// </param>
		...
		/// <para>仅当目标要素参数值为要素类或图层时，此参数才处于活动状态。</para>
		...
		/// <para>仅当将位置类型参数设置为坐标且目标要素参数值为要素类或图层时，此参数才处于活动状态。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[Field()]
		[GPFieldDomain()]
		...
		/// <para>如果完整地址储存在输入表的一个字段中，例如 303 Peachtree St NE, Atlanta, GA 30308，请选择单个字段。 如果将常规美国地址的输入地址拆分成“地址”、“城市”、“州”和“邮政编码”等多个字段，请选择多个字段。</para>
		/// <para>仅当将位置类型参数设置为地址且目标要素参数值为要素类或图层时，此参数才处于活动状态。</para>
		...
		/// <para>仅当已选中更新现有目标要素参数且目标要素参数值为要素类或图层时，此参数才会处于活动状态。</para>
