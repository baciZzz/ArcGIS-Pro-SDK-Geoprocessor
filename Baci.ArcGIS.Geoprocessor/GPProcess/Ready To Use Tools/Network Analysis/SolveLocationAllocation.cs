...
namespace Baci.ArcGIS.Geoprocessor.ReadyToUseTools
{
	...
	/// <para>求解位置分配</para>
	/// <para>从一组输入位置中选择一个或多个最佳位置，方法是向输入设施点分配请求点，并满足将大部分请求点分配到设施点并最小化总行程的条件。</para>
	/// </summary>
	public class SolveLocationAllocation : AbstractGPProcess
	{
		...
		/// <para>最大化覆盖范围—将设施点设置在适当的位置，以使尽可能多的请求被分配到所求解的设施点的阻抗中断内。“最大化覆盖范围”常用于定位消防站、警察局和 ERS 中心，因为紧急救援服务通常需要在指定响应时间内到达所有请求点位置。 请注意，具有准确精密的数据，以确保分析结果能够准确的为真实世界建模，这对所有组织都十分重要，对紧急救援服务尤为关键。与在店内就餐的比萨饼店相反，比萨外卖业务会试图将店址设在一定车程时间范围内可以覆盖最多人口的位置。 定购比萨外卖的人通常不关心比萨饼店的远近；他们只关心比萨是否能够在店家所说的时间内送达。 因此，比萨外卖业务会从其宣称的送货时间中减去比萨制作时间，并针对最大化覆盖范围这一问题类型进行求解，以选出在覆盖区域中能够争取到最多潜在顾客的候选设施点。 （在店内就餐的比萨饼店的潜在顾客受距离的影响更大，因为他们要前往餐馆就餐，因此人流量最大化和市场份额两种问题类型更适合于此种情况。）以下列表描述了“最大化覆盖范围”这一问题类型对请求的处理方法：</para>
		...
		/// <para>设施点到请求点—行驶方向是从请求点指向设施点。 这是默认设置。</para>
		...
		/// <para>本地地理位置—时间参数是指设施点或请求点所处的时区。 如果行驶方向是从设施点到请求点，此为设施点所处的时区。 如果行进方向是从请求点到设施点，此为请求点所处的时区。</para>
