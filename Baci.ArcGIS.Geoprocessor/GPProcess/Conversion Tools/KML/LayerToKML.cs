...
namespace Baci.ArcGIS.Geoprocessor.ConversionTools
{
	...
	/// <para>图层转 KML</para>
	/// <para>用于将要素或栅格图层转换为 KML 文件，其中包含 Esri 几何和符号系统的转换。该文件采用 ZIP 压缩方式压缩，具有 .kmz 扩展名，并且可以由任何 KML 客户端读取，包括 ArcGIS Earth、ArcGlobe 和 Google Earth。</para>
	/// </summary>
	public class LayerToKML : AbstractGPProcess
	{
		...
		/// <para>选中 – 输出 KML 文件将是表示源图层中的栅格或矢量要素的单一合成图像。栅格以 KML GroundOverlay 形式叠加在地形上方。使用该选项可减小输出 KMZ 文件的大小。使用该选项时，KML 中的单个要素和图层将不可选择。</para>
