...
namespace Baci.ArcGIS.Geoprocessor.ConversionTools
{
	...
	public class MapToKML : AbstractGPProcess
	{
		...
		/// <para>导出地图中的每个图层时所使用的比例。</para>
		/// <para>此参数对任何比例可变选项都很重要，例如图层可见性或按比例渲染。如果图层在输出比例下不可见，则其不会包含在输出 KML 中。如果没有比例可变选项，则可使用任何值（例如 1）。</para>
		...
		/// <para>指定输出 KML 是包含单个合成图像还是单独的图层。</para>
		/// <para>选中 - 输出 KML 文件包含将地图中的所有要素合成为单个栅格图像的单个图像。栅格以 KML GroundOverlay 形式悬在地形上方。此选项可减小输出 KML 文件的大小。选择此选项时，KML 中的各个要素和图层将不可选择。</para>
		/// <para>未选中 - 输出 KML 包含单独的单个图层。这是默认设置。可通过矢量转栅格参数确定是以栅格的形式还是以矢量和栅格组合的形式返回图层。</para>
		...
		/// <para>选中 - 矢量图层在 KML 输出中转换为单独的栅格图像。正态栅格图层也将被添加到 KML 输出中。各输出 KML 栅格图层为可选状态，并且可以在特定 KML 客户端中调整其透明度。</para>
		/// <para>未选中 - 矢量图层保留为 KML 矢量。这是默认设置。</para>
		...
		/// <para>输出 KML 文档中所有栅格的设备分辨率。典型的屏幕分辨率是 96 dpi。如果地图中的数据支持高分辨率并且 KML 需要高分辨率，则考虑增加该值。可将该参数与返回图像的大小（像素）参数一起用于控制输出图像分辨率。默认值为 96。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPLong()]
		[Category("Output Image Properties")]
		...
		/// <para>选中 - 将覆盖输入要素的 z 值，并在创建 KML 时将要素强制固定于地面。要素将叠加在地形上。此选项适用于不具有 z 值的要素。这是默认设置。</para>
		/// <para>未选中 - 在创建 KML 时使用输入要素的 z 值。将在 KML 客户端中相对于海平面绘制要素。</para>
