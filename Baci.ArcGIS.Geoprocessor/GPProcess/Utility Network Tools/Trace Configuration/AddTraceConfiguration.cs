...
namespace Baci.ArcGIS.Geoprocessor.UtilityNetworkTools
{
	...
	public class AddTraceConfiguration : AbstractGPProcess
	{
		...
		/// <para>用于指定针对特定类别或网络属性的追踪停止时间。 例如，针对生命周期状态属性等于特定值的要素，追踪停止。 此参数用于根据系统中定义的网络属性值设置终止符。 如果您要使用多个属性，可使用组合方法选项来定义 And 或 Or 条件。</para>
		/// <para>过滤器障碍组件如下：</para>
		...
		/// <para>平均值 - 输入值的平均值。</para>
		/// <para>计数 - 要素数。</para>
		/// <para>减 - 输入值之间的差值。子网控制器和循环追踪类型不支持减函数。</para>
		/// <para>例如，起点要素的值为 20。 下一个要素的值为 30。 如果使用 Minimum 函数，则结果为 20；使用 Maximum 函数，结果为 30；使用 Add 函数，结果为 50；使用 Average 函数，结果为 25；使用 Count 函数，结果为 2；使用 Subtract 函数，结果为 -10。</para>
