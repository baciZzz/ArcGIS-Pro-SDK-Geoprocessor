...
namespace Baci.ArcGIS.Geoprocessor.GeostatisticalAnalystTools
{
	...
	/// <para>半变异函数灵敏度</para>
	/// <para>此工具通过在原始值的某一百分比范围内更改模型的半变异函数参数（块金、偏基台和主/次变程）对预测值和关联的标准误差执行灵敏度分析。工具采用地统计模型源，以填充块金、偏基台和主/次变程的初始值。该工具的输出是一张表，指出使用了哪些参数值和生成的预测值与标准误差都是多少。如果模型的参数值变化很小但输出的波动很大，则基本不能相信输出。相反，如果输出中的变化很小，则可相信模型的预测并可根据它的输出做出决定。</para>
	/// </summary>
