...
namespace Baci.ArcGIS.Geoprocessor.ModelTools
{
	...
	public class SelectionExistsIfThenElse : AbstractGPProcess
	{
		...
		/// <para>指定用于匹配 SQL 表达式的记录字段值的选择条件。</para>
		/// <para>存在—检查记录中存在的字段值是否与 SQL 表达式匹配。这是默认设置。</para>
		/// <para>无选择内容—检查是否未选中任何与 SQL 表达式匹配的记录。</para>
		/// <para>全部选中—检查是否选中了所有与 SQL 表达式匹配的记录。</para>
		/// <para>等于—检查与 SQL 表达式匹配的记录的字段值是否等于计数值。</para>
		/// <para>介于—检查与 SQL 表达式匹配的记录，其字段值是否介于最小计数值与最大计数值之间。</para>
		/// <para>小于—检查与 SQL 表达式匹配的记录的字段值是否等于计数值。</para>
