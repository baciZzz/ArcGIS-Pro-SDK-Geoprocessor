...
namespace Baci.ArcGIS.Geoprocessor.ModelTools
{
	...
	public class IterateLayers : AbstractGPProcess
	{
		...
		/// <para>指定用于过滤图层的图层类型。 如果未指定图层类型，则将迭代所有受支持的图层类型。 可以使用多种图层类型来过滤图层。</para>
		...
		/// <para>指定用于过滤图层的工作空间类型。 如果未指定工作空间类型，则将迭代受支持的工作空间类型的所有字段。</para>
		/// <para>仅当图层类型参数设置为要素图层、栅格图层或表视图时，才启用工作空间类型参数。</para>
		/// <para>大数据连接—将迭代大数据连接工作空间中的图层。</para>
		...
		/// <para>指定用于过滤图层的要素类型。 如果未指定要素类型，则将迭代所有受支持的要素类型。</para>
		/// <para>注记—将迭代注记要素类。</para>
		/// <para>维度—将迭代尺寸注记要素类。</para>
		...
		/// <para>当工作空间类型参数设置为栅格时，用于过滤栅格图层的栅格格式类型。 如果未指定栅格类型，则将迭代受支持栅格类型的所有图层。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPMultiValue()]
		[GPCodedValueDomain()]
		...
		/// <para>指定是否使用图层可见性过滤图层。 如果未指定图层可见性，则将迭代所有图层。</para>
		/// <para>可见—将使用图层可见性过滤图层，并将迭代可见图层。</para>
		/// <para>不可见—不会使用图层可见性过滤图层，并将迭代不可见的图层。</para>
		...
		/// <summary>
		/// <para>State</para>
		/// <para>指定将用于过滤图层的图层状态。 如果该参数设置为无效，则将返回源路径图层损坏的图层。 如果未指定图层状态，则将迭代所有图层。</para>
