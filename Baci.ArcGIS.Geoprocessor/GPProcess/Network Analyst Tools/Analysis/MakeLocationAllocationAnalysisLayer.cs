...
namespace Baci.ArcGIS.Geoprocessor.NetworkAnalystTools
{
	...
	public class MakeLocationAllocationAnalysisLayer : AbstractGPProcess
	{
		...
		/// <summary>
		/// <para>Tool Display Name : 创建位置分配分析图层</para>
		/// </summary>
		public override string DisplayName() => "创建位置分配分析图层";
		...
		/// <para>为衰减函数类型参数中指定的方程提供参数值。当衰减函数的类型为线性时会忽略参数值。对于幂和指数衰减参数，应设置非零值。</para>
		/// <para>如果设置请求点的 ImpedanceTransformation 属性，则该属性会逐个请求点覆盖分析图层的衰减函数参数值属性。这时，您可能要针对城镇居民和乡村居民使用不同的衰减函数。可通过为分析图层设置阻抗变换以匹配乡村居民的衰减函数，同时为城镇地区中的各个请求点设置阻抗变换以匹配城镇居民的衰减函数，来执行建模。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPDouble()]
		...
		/// <para>指示出发的时间和日期。出发时间可以来自设施点或请求点，取决于行驶方向是从请求点到设施点还是从设施点到请求点。</para>
		...
		/// <para>Time Zone</para>
		/// <para>时间参数的时区。</para>
		/// <para>各位置的本地时间—时间参数是指设施点或请求点所处的时区。如果行驶方向是从设施点到请求点，此为设施点所处的时区。如果行进方向是从请求点到设施点，此为请求点所处的时区。这是默认设置。</para>
