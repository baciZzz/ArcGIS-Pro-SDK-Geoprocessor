...
namespace Baci.ArcGIS.Geoprocessor.NetworkAnalystTools
{
	...
	public class MakeServiceAreaAnalysisLayer : AbstractGPProcess
	{
		...
		/// <summary>
		/// <para>Cutoffs</para>
		/// <para>将使用所选出行模式使用的抗阻属性单位计算服务区范围。 例如，分析行驶时间时，中断值 10 表示生成的服务区将代表 10 分钟行驶区域内可送达的区域。</para>
		...
		/// <para>圆环—各个面将仅包括连续中断值之间的区域。 其将不会包括设施点和任何较小中断值之间的区域。 例如，如果创建 5 分钟和 10 分钟服务区，5 分钟服务区面将包含 0 到 5 分钟内可到达的区域，而 10 分钟服务区面则包括 5 到 10 分钟内可到达的区域。 这是默认设置。</para>
		/// <para>圆盘—各个面将包含从设施点到中断值内可到达的区域，其中包括较小中断值内可到达的区域。 例如，如果创建 5 分钟和 10 分钟服务区，则 10 分钟服务区面将包含 5 分钟服务区面内的区域。</para>
		...
		/// <para>从服务区面中排除网络源时，不会阻止遍历这些源。 从服务区面中排除网络源仅影响服务区的面形状。 要阻止遍历给定网络源，必须在定义网络数据集时创建适当的限制。</para>
		...
		/// <para>&lt;para/&gt;分析过程中要累积的成本属性的列表。 这些累积属性仅供参考；求解程序仅使用求解分析时指定的出行模式所使用的成本属性。</para>
