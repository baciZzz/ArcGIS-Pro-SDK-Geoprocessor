...
namespace Baci.ArcGIS.Geoprocessor.NetworkAnalystTools
{
	...
	/// <para>求解车辆配送问题</para>
	/// <para>创建车辆配送 (VRP) 网络分析图层，设置分析属性，并求解分析，是设置 VRP Web 服务的理想手段。 VRP 分析图层用于查找车队的最佳路径。</para>
	/// </summary>
	[Obsolete()]
	public class SolveVehicleRoutingProblem : AbstractGPProcess
	{
		...
		/// <para>站点要素集具有一个关联的属性表。 下面列出了属性表中的字段及其描述。</para>
		/// <para>ObjectID:</para>
		...
		/// <para>当任一时间窗字段包含当时的日期时，将忽略默认日期。 为避免在这种情况下出现错误，请将 Depots、Routes、Orders 和 Breaks 中所有时间窗的格式设置为也包含日期和时间。</para>
		...
		/// <para>方位角数据通常会从配有 GPS 接收器的移动设备自动发送。 如果正在加载移动停靠点（例如行人或车辆），请尝试包括方位角数据。</para>
		...
		/// <para>单位为度，并且默认值为 30。值必须大于零且小于 180。</para>
		/// <para>值为 30 表示，Network Analyst 尝试在边上添加网络位置时，在边的每一侧（左侧和右侧）的两个数字化方向上都将生成一个 15º 的可接受方位角值。</para>
		/// <para>有关详细信息，请参阅方位角和 BearingTol。</para>
		/// <para>NavLatency:</para>
		/// <para>如果 Bearing 和 BearingTol 也具有值，则该字段只在求解过程中使用；但是，即使当 Bearing 和 BearingTol 中有值时，NavLatency 值的输入也是可选的。NavLatency 表示 GPS 信息从移动的车辆上发送到服务器以及车辆导航设备接收到处理后路径这两个时刻之间预期要花费的时间。 NavLatency 的时间单位与时间属性参数指定的成本属性的单位相同。</para>
		/// </param>
		/// <param name="Routes">
		/// <para>Routes</para>
		/// <para>可用于给定的车辆配送 (VRP) 问题的路径。 路径指定了车辆和驾驶员的特征；求解后，其还可以表示站点和停靠点之间的路径。</para>
		/// <para>路径可以具有开始和结束站点服务时间、固定或灵活的起始时间、基于时间的运行成本、基于距离的运行成本、多个容量、对驾驶员工作时间的各种约束等等。</para>
		/// <para>路径记录集具有多个属性。 下面列出了属性表中的字段及其描述。</para>
		/// <para>Name:</para>
		/// <para>路径的名称。 名称必须唯一。</para>
		/// <para>如果字段值为空，则 Network Analyst 会在求解时生成唯一的名称。 因此，在大多数情况下，可自行选择是否输入值。 但是，如果您的分析中包括向路径预分配的中断、路径货物补给点、按区域配送或停靠点，则您必须输入名称，因为在这些情况下路径名称将用作外键。 请注意，路径名称不区分大小写。</para>
		...
		/// <para>如果路径正在进行配送并且 StartDepotName 为空，则假设在路径开始前，在一个虚拟站点处进行装货。 如果路径不具有货物补给点，则它的配送停靠点（Orders 类中 DeliveryQuantities 值为非零的停靠点）会在起始站点或虚拟站点处进行装货。 如果路径具有更新访问，则只有第一个更新访问之前的配送停靠点才会在起始站点或虚拟站点处进行装货。</para>
		...
		/// <para>起始和结束站点处的服务时间是固定值（由 StartDepotServiceTime 和 EndDepotServiceTime 字段值指定），因此不必考虑路径的实际载荷。 例如，在起始站点处装载车辆所花费的时间取决于订单大小。 因此，可为站点服务时间分配与货车满载或货车平均装载对应的值，或者也可以设置自行估计的时间值。</para>
		...
		/// <para>为了说明，假设在一幢高层建筑物中有五个重合停靠点，而且可通过三条不同的路径来为它们提供服务。 这意味着将产生三个到达/离开延迟；也就是说，三名驾驶员需要分别寻找停车位并进入同一栋建筑物。 不过，如果可以仅通过一条路径来为这些停靠点提供服务，则只有一名驾驶员需要寻找停车位并进入该建筑物，这样只会产生一个到达/离开延迟。 由于 VRP 求解程序会尝试将成本降至最低，所以它将尝试限制到达/离开延迟，因而会选择使用单一路径。 （请注意，特殊要求、时间窗或容量等其他约束可能要求发送多条路径。）</para>
		/// <para>该字段值的单位由时间字段单位参数（Python 中的 time_units）指定。</para>
		/// <para>Capacities:</para>
		/// <para>车辆的最大容量。 可以按任何度量单位（如重量、体积或数量）来指定容量。 也可以指定多个度量单位，例如，重量和体积。</para>
		/// <para>输入未指明单位的容量。 例如，假定您车辆的最大载重为 40,000 磅；您可输入 40000。 您需要记住该值的单位是磅，以供将来参考。</para>
		...
		/// <para>鉴于以下原因，记住单位和单位顺序非常重要：第一，您可以在稍后重新解释信息；第二，您可以在停靠点的 DeliveryQuantities 和 PickupQuantities 字段中输入正确的值。 为了详细描述第二点，请注意 VRP 求解程序会同时引用 Capacities、DeliveryQuantities 和 PickupQuantities，以确保路径不会超载。 由于字段中不能输入单位，Network Analyst 不能转换单位，因此您需要以相同的单位和单位顺序在这三个字段中输入值，以确保能正确地解释值。 如果在这三个字段的任意字段中使用的单位不同或顺序发生变化，那么您将得到意外结果，但不会收到警告消息。 因此，建议您事先设置单位和单位顺序标准，且后续在这三个字段中输入值时始终参考此标准。</para>
		...
		/// <para>每单位加班工作时间产生的货币成本。 如果 OvertimeStartTime 也为空，则只能包含空值。 否则，它必须为大于 CostPerUnitTime 的正值。</para>
		...
		/// <para>为了说明什么是特殊要求及其工作方式，假设草坪护理及树木修剪公司都有一部分停靠点需要使用斗式铲车来修剪所有树木。 公司可以在 SpecialtyNames 字段中为这些停靠点输入 BucketTruck 来表示其特殊要求。 SpecialtyNames 对于其他停靠点，SpecialtyNames 将留为空值。 同样，公司也可以在带液压吊杆的铲车路径的 SpecialtyNames 字段中输入 BucketTruck。 对于其他路径该字段将留为空值。 求解时，VRP 求解程序会将无任何特殊要求的停靠点分配给任意路径，而将需要斗式铲车的停靠点分配给有斗式铲车的路径。</para>
		...
		/// <para>此距离单位无需与网络距离属性（Python 中的 distance attribute）的距离单位相匹配。</para>
		/// <para><see cref="DistanceUnitsEnum"/></para>
		/// </param>
		/// <param name="NetworkDataset">
		/// <para>Network Dataset</para>
		/// <para>将对其执行车辆配送 (VRP) 分析的网络数据集。 由于 VRP 求解程序要尽量缩短配送时间，因此网络数据集必须具有基于时间的成本属性。</para>
		/// </param>
		/// <param name="OutputWorkspaceLocation">
		/// <para>Output Geodatabase Workspace</para>
		/// <para>将创建输出要素类的文件地理数据库或内存工作空间。 此工作空间必须已经存在。 默认输出工作空间位于内存中。</para>
		/// </param>
		...
		/// <para>包含了路径所访问的停靠点的要素类的名称。 此要素类包括站点停靠点、停靠点和休息点。</para>
		/// </param>
		/// <param name="OutputRoutesName">
		/// <para>Output Routes Name</para>
		/// <para>包含了分析路径的要素类的名称。</para>
		/// </param>
		...
		/// <summary>
		/// <para>Breaks</para>
		/// <para>给定车辆配送 (VRP) 问题中路径的休息时段或中断。 一个中断只与一条路径相关联，而且可在以下情况下获得：完成停靠点服务后、去往停靠点的途中或在为停靠点提供服务之前。 中断具有一个起始时间和持续时间，该时间段内可能会为驾驶员支付报酬，也可能不支付。 休息开始时可通过三种方式来建立：使用时间窗、最长行驶时间或最长工作时间。</para>
		/// <para>中断记录集具有关联的属性。 下面列出了属性表中的字段及其描述。</para>
		/// <para>&lt;para/&gt;ObjectID:</para>
		/// <para>系统管理的 ID 字段。</para>
		/// <para>RouteName:</para>
		/// <para>休息点所应用到的路径的名称。 一个休息点仅分配给一条路径，但可将多个休息点分配给同一路径。</para>
		/// <para>该字段是 Routes 类中 Name 字段的外键，并且不能具有空值。</para>
		...
		/// <para>零值表示不能与时间窗发生冲突；即时间窗是硬性的。 非零值指定最长延迟时间；例如，中断可在其时间窗结束后最多 30 分钟内开始，但会按照时间窗冲突重要性参数（Python 中的 time_window_factor）对延迟进行惩罚。</para>
		/// <para>该属性可以为空；TimeWindowStart 和 TimeWindowEnd 为空值表示对允许的冲突时间没有限制。 如果 MaxTravelTimeBetweenBreaks 或 MaxCumulWorkTime 中存在值，则 MaxViolationTime 必须为空。</para>
		/// <para>该字段值的单位由时间字段单位参数（Python 中的 time_units）指定。</para>
		...
		/// <para>该字段用于限制可在驾驶多长时间之后才需要中断。 例如，如果分析的时间字段单位参数（Python 中的 time_units）设为分钟，而且 MaxTravelTimeBetweenBreaks 的值为 120，则司机将在驾驶两个小时之后中断驾驶以得到休息。 如果要再驾驶两个小时后中断一次，则第二个中断的 MaxTravelTimeBetweenBreaks 字段值应为 120。</para>
		...
		/// <para>该字段用于限制可在工作多长时间之后才需要中断。 例如，如果时间字段单位参数（Python 中的 time_units）设为分钟，而且 MaxCumulWorkTime 的值为 120，ServiceTime 的值为 15，则司机将在工作两个小时之后获得 15 分钟的休息时间。</para>
		/// <para>继续以上一个示例来进行说明，假设工作了三个小时之后又需要休息。 那么，要指定该休息点，需要输入 315（5 小时 15 分钟）作为第二个休息点的 MaxCumulWorkTime 值。 这个数字包括前一个休息点的 MaxCumulWorkTime 值和 ServiceTime 值，以及准许进行第二次休息之前的另外三个小时工作时间。 为避免过早经过最长工作时间休息点，请牢记：此类休息点是从路径的起始点开始累积工作时间，并且工作时间包括在之前访问的站点、停靠点和休息点处的服务时间。</para>
		...
		/// <para>Sequence:</para>
		/// <para>该输入字段用于指示休息点在其路径上的顺序。 该字段可包含空值。 输入顺序值应为正且对于各路径均唯一（在货物补给点、停靠点和休息点之间共享），但是不需要从 1 开始，也不需要连续。</para>
		/// <para>求解程序会修改 sequence 字段。 执行求解后，该字段会包含休息点在其路径上的 sequence 值。 路径的输出顺序值在货物补给点、停靠点和休息点之间分配；从 1 开始（在起始站点处）；并且是连续的。</para>
		/// </summary>
		...
		/// <para>指定支持时间窗的重要性。 下面列出了这些字段及其描述。</para>
		...
		/// <para>路径区要素集具有一个关联属性表。 下面列出了属性表中的字段及其描述。</para>
		/// <para>ObjectID:</para>
		/// <para>系统管理的 ID 字段。</para>
		/// <para>Shape:</para>
		/// <para>指示网络分析对象地理位置的几何字段。</para>
		/// <para>RouteName:</para>
		/// <para>该区域所应用到的路径的名称。 按区域配送最大可覆盖一条关联路径。 该字段不能包含空值，而且是 Routes 要素图层中 Name 字段的外键。</para>
		...
		/// <para>货物补给点记录集具有关联属性。 下面列出了属性表中的字段及其描述。</para>
		/// <para>ObjectID:</para>
		/// <para>系统管理的 ID 字段。</para>
		/// <para>DepotName:</para>
		/// <para>进行该更新时所在站点的名称。 该字段不能包含空值，而且是 Depots 要素图层中 Name 字段的外键。</para>
		/// <para>RouteName:</para>
		/// <para>该更新所应用到的路径的名称。 该字段不能包含空值，而且是 Routes 要素图层中 Name 字段的外键。</para>
		/// <para>ServiceTime:</para>
		...
		/// <para>有时，要求停靠点的接收和配送是成对的。 例如，快递公司可能需要让路径从一个停靠点接收高优先级包裹并送往另一个停靠点，而不返回站点或分拣站，以减少送货时间。 可以通过使用停靠点对，按照相应的顺序将这些相关的停靠点分配给同一路径。 此外，还可以对包裹在车辆上停留的时间进行限制；例如，包裹可能是血液样本，必须在两个小时内从医生办公室运送到实验室。</para>
		/// <para>停靠点对记录集具有关联属性。 下面列出了属性表中的字段及其描述。</para>
		/// <para>ObjectID:</para>
		/// <para>系统管理的 ID 字段。</para>
		/// <para>FirstOrderName:</para>
		/// <para>停靠点对中第一个停靠点的名称。 该字段是 Orders 要素图层中 Name 字段的外键。</para>
		/// <para>SecondOrderName:</para>
		/// <para>停靠点对中第二个停靠点的名称。 该字段是 Orders 要素图层中 Name 字段的外键。</para>
		...
		/// <para>未选中 - 网络位置将放置在网络的所有元素上。 使用了该选项，如果添加的网络位置被放置在受限元素上，那么在求解过程中，可能无法到达这些网络位置。</para>
		...
		/// <para>路径几何的简化距离。</para>
		/// <para>简化将保留路径上定义路径基本形状所需的关键点（例如交点处的转弯）而删除其他点。 指定的简化距离为简化线偏离原始线的最大允许偏移。 对线进行简化将减少折点的数量，并且往往能够减少绘制时间。</para>
		/// <para>将出行模式（Python 中的 travel_mode）设置为除自定义外的其他值时会覆盖此参数的值。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPLinearUnit()]
		...
		/// <para>如果要将此工具作为独立服务器服务的一部分进行发布，那么必须确保与您所选语言相对应的 ArcGIS 语言包已经安装到服务器上，这样才能够保证工具正常工作。 此外，如果计算机上未安装语言包，则该语言将不会出现在下拉列表中；但是，可以输入语言代码。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
		[GPCodedValueDomain()]
		...
		/// <para>NA Navigation—针对车辆内导航设备设计的转弯方向</para>
		...
		/// <para>在任何一种情况下，将返回独立的表和要素类。 不过，服务器管理员可能希望选择输出一个网络分析图层，以便可以使用 ArcGIS Desktop 环境中的 Network Analyst 控件调试工具的设置和结果。 这会使调试过程变得更加容易。</para>
		...
		/// <para>除了限制路径的最大数量之外，您还可以限制向分析中添加的停靠点或点障碍的数量。 控制问题大小的另一种方法是设置要素（通常为街道要素，即线或面障碍相交的要素）的最大数量。 最后一种方法是：当停靠点在地理上分散超过给定的直线距离时，即使用户选择不应用等级，仍强制执行等级求解。</para>
		...
		/// <para>FORCE HIERARCHY BEYOND DISTANCE—使用网络等级求解车辆配送前，停靠点之间的最大直线距离。 此值的单位与距离字段单位参数中指定的相同。如果网络没有等级属性，将忽略此约束。 如果选中在分析中应用等级，将总是使用等级。 如果未选中在分析中应用等级参数且此约束为空值，将不强制执行等级。</para>
		...
		/// <para>为工具支持的以下输入日期时间字段指定时区：对于停靠点，TimeWindowStart1、TimeWindowEnd1、TimeWindowStart2、TimeWindowEnd2、InboundArriveTime 和 OutboundDepartTime；对于站点，TimeWindowStart1、TimeWindowEnd1、TimeWindowStart2 和 TimeWindowEnd2；对于路径，EarliestStartTime 和 LatestStartTime；对于休息点，TimeWindowStart 和 TimeWindowEnd。</para>
		/// <para>地理定位—与停靠点或站点相关的日期时间值位于停靠点和站点所在的时区内。 对于路径，日期时间值基于路径的起始站点所在的时区。 如果路径没有起始站点，则路径上的所有停靠点和站点必须位于一个时区中。 对于中断点，日期时间值基于路径的时区。 例如，如果站点所在的区域实行东部标准时间并且第一个时间窗值（指定为 TimeWindowStart1 和 TimeWindowEnd1）为 8 a.m. 和 5 p.m.，则在东部标准时间中时间窗值将被视为 8 a.m. 和 5 p.m.。</para>
