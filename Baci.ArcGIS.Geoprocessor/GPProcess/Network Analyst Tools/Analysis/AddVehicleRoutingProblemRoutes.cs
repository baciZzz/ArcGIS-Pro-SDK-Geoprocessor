...
namespace Baci.ArcGIS.Geoprocessor.NetworkAnalystTools
{
	...
	public class AddVehicleRoutingProblemRoutes : AbstractGPProcess
	{
		...
		/// <para>路径的起始站点的名称。如果起始站点名称值为空，则路径会将分配的第一个停靠点作为起始点。车辆的起始位置未知或者与您的问题不相关时，可以忽略起始站点。但是，当起始站点名称值为空时，则终止站点名称值不能同时为空。如果停靠点或站点跨多个时区，则不允许使用虚拟起始站点。</para>
		/// </summary>
		...
		/// <para>求解程序通过将该参数与起始站点的时间窗（在 Depots 图层中由 TimeWindowStart 提供）结合使用来确定可行的路径开始时间。该参数的默认仅时间值是 8:00:00 a.m.，解释为分析层的 Default Date 属性给定的日期的上午 8:00:00。如果未指定任何值，则将使用默认值。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPDate()]
		...
		/// <para>路径允许的最晚开始时间。该参数的默认仅时间值是 10:00:00 a.m.，解释为分析层的 Default Date 属性提供的日期的上午 10:00:00。如果未指定任何值，则将使用默认值。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPDate()]
