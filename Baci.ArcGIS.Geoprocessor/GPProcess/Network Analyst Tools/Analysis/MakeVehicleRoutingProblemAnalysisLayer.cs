...
namespace Baci.ArcGIS.Geoprocessor.NetworkAnalystTools
{
	...
	/// <para>用于创建车辆配送 (VRP) 网络分析图层并设置其分析属性。VRP 分析图层可用于在使用一支车队时对一组路径进行优化。该图层可通过本地网络数据集进行创建，也可通过在线托管服务或门户托管服务进行创建。</para>
	/// </summary>
	public class MakeVehicleRoutingProblemAnalysisLayer : AbstractGPProcess
	{
		...
		/// </summary>
		public override object[] Parameters() => new object[] { NetworkDataSource, LayerName, TravelMode, TimeUnits, DistanceUnits, DefaultDate, TimeZoneForTimeFields, LineShape, TimeWindowFactor, ExcessTransitFactor, GenerateDirectionsOnSolve, SpatialClustering, OutNetworkAnalysisLayer };
		...
		/// <para>各位置的本地时间— 与停靠点或站点相关的日期时间值位于停靠点和站点所在的时区内。对于路径，日期时间值基于路径的起始站点所在的时区。如果路径没有起始站点，则路径上的所有停靠点和站点必须位于一个时区中。对于休息点，日期时间值基于路径的时区。这是默认设置。</para>
		...
		/// <para>中速—求解程序在满足时间窗和减少总体解决方案成本之间寻求一种平衡。这是默认设置。</para>
		...
		/// <para>中速—求解程序在减少超出行驶时间和减少总体解决方案成本之间寻求一种平衡。这是默认设置。</para>
