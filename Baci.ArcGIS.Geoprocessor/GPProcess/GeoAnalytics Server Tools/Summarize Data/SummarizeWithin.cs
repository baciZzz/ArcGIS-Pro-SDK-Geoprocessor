...
namespace Baci.ArcGIS.Geoprocessor.GeoAnalyticsServerTools
{
	...
	public class SummarizeWithin : AbstractGPProcess
	{
		...
		/// <para>正方形—图格大小表示方形的高度。这是默认设置。</para>
		...
		/// <para>计数—将计算乘以面内汇总图层的比例后的每个字段的计数。</para>
		/// <para>总和—将计算每个字段内加权值的总和，其中所应用的权重是面内汇总图层的比例。</para>
		/// <para>平均值—将计算每个字段内加权值的平均值，其中所应用的权重是面内汇总图层的比例。</para>
		/// <para>最小值—将计算每个字段中的最小加权值，其中所应用的权重是面内汇总图层的比例。</para>
		/// <para>最大值—将计算每个字段中的最大加权值，其中所应用的权重是面内汇总图层的比例。</para>
		/// <para>范围—将计算最小值与最大值之间的差值。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPValueTable()]
		[GPCompositeDomain()]
