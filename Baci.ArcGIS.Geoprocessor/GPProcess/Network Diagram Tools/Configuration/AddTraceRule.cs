...
namespace Baci.ArcGIS.Geoprocessor.NetworkDiagramTools
{
	...
	public class AddTraceRule : AbstractGPProcess
	{
		...
		/// <para>最短路径— 当规则开始沿起点之间的最短路径查找要素时，系统将从逻辑示意图中当前指定为起点的 trace network 要素执行最短路径追踪。遍历路径的成本取决于为最短路径网络属性名称参数 值设置的网络属性，而与流向无关。</para>
		/// </param>
		...
		/// <para>针对 utility network 运行追踪的域网络名称。运行子网、上溯和下溯追踪类型时，需要此参数。</para>
		/// </summary>
		...
		/// <summary>
		/// <para>Tier</para>
		/// <para>追踪针对 utility network 开始的层名称。运行已连接追踪类型时可以使用此参数； 运行子网、上溯和下溯追踪类型时必须使用此参数。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
		...
		/// <para>utility network 输入层流向的目标层名称。如果上溯和下溯追踪的此参数丢失，当到达起始子网边界时，上述追踪将停止。该参数可以使此类追踪在层级结构中继续向上或向下延伸。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
		...
		/// <para>要强制使用的可遍历性的类型。可遍历性范围指明是否在交汇点、边或这两处强制使用可遍历性。例如，如果定义了用于停止追踪的条件障碍，其中 Device Status 等于 Open 并将遍历范围仅设置为边，则即使追踪遇到开路设备，追踪也不会停止，因为 Device Status 仅适用于交汇点。换言之，此参数会为追踪指明是否要忽略交汇点和/或边。</para>
		...
		/// <para>过滤器障碍组件如下：</para>
		/// <para>名称 - 选择按类别或系统中定义的任何网络属性进行过滤。</para>
		/// <para>运算符 - 从大量不同的运算符中进行选择。</para>
		/// <para>类型 - 从名称参数指定的值中选择特定值或网络属性。</para>
		/// <para>值 - 提供会基于运算符值导致终止的输入属性类型的特定值。</para>
		/// <para>使用以下内容进行结合 - 如果要添加多个属性，则设置此值。您可以选择使用 And 或 Or 条件来对它们进行组合。</para>
		...
		/// <para>属性 - 选择按系统中定义的任何网络属性进行过滤。</para>
		/// <para>运算符 - 从大量不同的运算符中进行选择。</para>
		/// <para>值 - 提供将导致终止的输入属性类型（若发现）的特定值。</para>
		/// <para>使用局部值 - 计算每个方向的值，而不是整体全局值。例如计算形状长度总和的函数障碍，其中，如果值大于或等于 4，则追踪终止。在全局情况下，遍历两条值为 2 的边之后，形状长度总和即已达到 4，因此追踪会停止。如果使用局部值，每条路径上的局部值会变化，否则追踪将继续。</para>
		/// <para>选中 - 使用局部值。</para>
		/// <para>未选中 - 使用全局值。这是默认设置。</para>
		...
		/// <para>Average - 输入值的平均值</para>
		/// <para>Count - 要素数目。</para>
		/// <para>Subtract - 各值之间的差值。子网控制器和循环追踪类型不支持减法函数。</para>
		/// <para>例如，起点要素的值为 20。下一个要素的值为 30。如果使用 Minimum 函数，则结果为 20。使用 Maximum 函数，结果为 30；使用 Add 函数，结果为 50；使用 Average 函数，结果为 25；使用 Count 函数，结果为 2；使用 Subtract 函数，结果为 -10。</para>
		...
		/// <para>指定是否将追踪具有不确定或未初始化流向的追踪网络要素。此参数仅在于追踪网络上运行上溯追踪或下溯追踪时使用。</para>
		/// <para>选中 - 追踪中包含具有不确定流向或未初始化流向的 trace network 要素。</para>
		/// <para>未选中 - 不包含具有不确定流向或未初始化流向的 trace network 要素。这是默认设置。</para>
		...
		/// <para>指定追踪网络的路径的方向。遍历路径的成本取决于为路径网络属性名称设置的值。此参数仅在运行路径追踪类型时使用。</para>
		/// <para>NO_DIRECTION—查找两个起点之间的路径，无论流向如何。这是默认设置。</para>
		...
		/// <para>用于为 trace network 计算路径的网络属性。运行最短路径追踪类型时，使用数字网络属性（例如形状长度）计算最短路径。基于成本和距离的路径都可以进行计算。运行最短路径追踪时，需要此参数。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
		[Category("Advanced Options")]
