...
namespace Baci.ArcGIS.Geoprocessor.AnalysisTools
{
	...
	public class Buffer : AbstractGPProcess
	{
		...
		/// <summary>
		/// <para>Side Type</para>
		/// <para>指定将在输入要素的哪一侧进行缓冲。</para>
		/// <para>完整—对于线输入要素，将在线两侧生成缓冲区。对于面输入要素，将在面周围生成缓冲区，并且这些缓冲区将包含并叠加输入要素的区域。对于点输入要素，将在点周围生成缓冲区。这是默认设置。</para>
		/// <para>左—对于线输入要素，将在线的拓扑左侧生成缓冲区。此选项对于面输入要素无效。</para>
		/// <para>右—对于线输入要素，将在线的拓扑右侧生成缓冲区。此选项对于面输入要素无效。</para>
		/// <para>从缓冲区中排除输入面—对于面输入要素，仅在输入面的外部生成缓冲区（输入面内部的区域将在输出缓冲区中被擦除）。此选项对于线输入要素无效。</para>
		...
		/// <para>平面—缓冲区的末端很平整或者为方形，并且在输入线要素的端点处终止。</para>
		/// <para>此可选参数不适用于 Desktop Basic 或 Desktop Standard 许可。</para>
