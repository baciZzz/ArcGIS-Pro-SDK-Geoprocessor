...
namespace Baci.ArcGIS.Geoprocessor.AnalysisTools
{
	...
	/// <para>邻近汇总</para>
	/// <para>查找输入图层中要素指定距离内的要素并计算邻近要素的统计数据。 距离的测量方式可采用直线距离、行驶时间距离（例如，10 分钟内）或行驶距离（5 公里内）。 行驶时间和行驶距离的测量要求您先使用网络分析权限登录到 ArcGIS Online 组织帐户，然后消耗配额。</para>
	/// </summary>
	public class SummarizeNearby : AbstractGPProcess
	{
		...
		/// <para>将被缓冲的点、线或面要素，以及用于汇总输入汇总要素的缓冲区。</para>
		/// </param>
		...
		/// <para>Distances</para>
		/// <para>距离值可定义搜索距离（例如直线、行驶、货运或步行距离）或行驶时间（行驶、货运或步行时间）。 将汇总处于您所输入距离内（包含该距离）的要素。</para>
		...
		/// <para>Geolocal—时间采用本地时区或输入要素所在的时区。 该选项会导致各时区中分析的起始时间有所变动。 这是默认设置。例如，将 geolocal 时间设置为上午 9:00 会致使东部时区内各点的行驶时间在东部时间上午 9:00 开始，同样中部时区内各点在中部时间上午 9:00 开始。 （开始时间与 UTC 时间或实际时间偏差一小时。）</para>
		/// <para>UTC—时间引用协调世界时间 (UTC)。 无论时区差异，所有点的开始时间均应同步。例如，将 UTC 时间设置为上午 9:00 会致使东部时区内各点的行驶时间在东部时间上午 4:00 开始，同样中部时区内各点在中部时间上午 3:00 开始。 （开始时间同步。）</para>
