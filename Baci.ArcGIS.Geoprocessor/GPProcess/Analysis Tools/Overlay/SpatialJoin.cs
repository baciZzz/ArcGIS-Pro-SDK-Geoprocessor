...
namespace Baci.ArcGIS.Geoprocessor.AnalysisTools
{
	...
	/// <para>空间连接</para>
	/// <para>根据空间关系将一个要素类的属性连接到另一个要素类的属性。 目标要素和来自连接要素的被连接属性写入到输出要素类。</para>
	/// </summary>
	public class SpatialJoin : AbstractGPProcess
	{
		...
		/// <para>目标要素的属性和被连接要素的属性将被传递到输出要素类。 但是，可以在字段映射参数中定义属性的子集。</para>
		/// </param>
		...
		/// <para>包含目标要素和连接要素的属性的新要素类。 默认情况下，目标要素的所有属性和被连接要素的属性都将被写入到输出。 但是，可通过字段映射参数来控制要传递的属性集。</para>
		/// </param>
		...
		/// <para>在具有相同空间关系的目标要素和连接要素之间存在一对多关系时，指定输出要素类中目标要素和连接要素之间的连接方式。</para>
		/// <para>一对一连接—如果找到与同一目标要素存在相同空间关系的多个连接要素，将使用字段映射合并规则对多个连接要素中的属性进行聚合。 例如，如果在两个独立的面连接要素中找到了同一个点目标要素，将对这两个面的属性进行聚合，然后将其传递到输出点要素类。 如果一个面要素的属性值为 3，另一个面要素的属性值为 7，且指定了“总和”合并规则，则输出要素类中的聚合值将为 10。 这是默认设置。</para>
		/// <para>一对多连接—如果找到多个与同一目标要素存在相同空间关系的连接要素，输出要素类将包含目标要素的多个副本（记录）。 例如，如果在两个独立的面连接要素中找到了同一个点目标要素，则输出要素类将包含目标要素的两个副本：分别包含两个面的属性记录。</para>
		...
		/// <para>选中 - 将在输出中保留所有目标要素（外部连接）。 这是默认设置。</para>
		/// <para>未选中 - 在输出要素类中仅保留那些与连接要素有指定空间关系的目标要素（内部连接）。 例如，如果将某个点要素类指定为目标要素，将某个面要素类指定为连接要素，并选择范围内作为匹配选项值，则输出要素类将仅包含那些位于面连接要素内部的目标要素。 非连接要素内部的目标要素将被从输出中排除。</para>
		...
		/// <para>Clementini 包含—该空间关系产生的结果与完全包含相同，但有一种情况例外：如果连接要素完全位于目标要素的边界上（没有任何一部分完全位于内部或外部），则不会匹配要素。Clementini 将边界面定义为用来分隔内部和外部的线，将线的边界定义为其端点，点的边界始终为空。</para>
		...
		/// <para>Clementini 位于—结果与范围内相同，但下述情况例外：如果连接要素中的全部要素均位于目标要素的边界上，则不会匹配要素。Clementini 将边界面定义为用来分隔内部和外部的线，将线的边界定义为其端点，点的边界始终为空。</para>
		...
		/// <para>如果连接要素与目标要素的距离在此范围内，则有可能进行空间连接。 仅当已指定空间关系时（匹配选项参数设置为相交、在某一距离范围内、在某一测地线距离范围内、质心在以下图层中的要素内、最近或最近测地线），搜索半径才有效。 例如，当搜索半径设置为 100 米且空间关系设置为在某一距离范围内时，将会连接距离目标要素 100 米以内的要素。 对于在某一距离范围内的三种关系而言，如果未指定搜索半径的值，则距离将为 0。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPLinearUnit()]
		...
		/// <para>添加到输出要素类中的字段的名称，其中包含目标要素和最近连接要素之间的距离。 仅当已指定空间关系时（匹配选项设置为最近或最近测地线），此选项才有效。 如果在搜索半径内没有任何匹配的要素，则此字段的值为 -1。 如果未指定字段名称，将不会向输出要素类中添加该字段。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
