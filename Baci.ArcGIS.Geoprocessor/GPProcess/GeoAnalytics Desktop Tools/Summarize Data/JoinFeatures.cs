...
namespace Baci.ArcGIS.Geoprocessor.GeoAnalyticsDesktopTools
{
	...
	public class JoinFeatures : AbstractGPProcess
	{
		...
		/// <para>The new feature class containing the target layer features with joined features.</para>
		/// </param>
		...
		/// <para>Geodesic Near—The features in the join features will be matched if they are within a specified distance of a target feature. The distance is measured geodesically. Specify a distance in the Spatial Near Distance parameter.</para>
		...
		/// <para>Near before—When a target time is before the join time but within a specified range of time from the join time, the target time is near before the join time.</para>
		/// <para>Near after—When a target time is after the join time but within a specified range of time from the join time, the target time is near after the join time.</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
		[GPCodedValueDomain()]
		...
		/// <para>For example, you could apply a join condition to features in which the HealthSpending attribute in the join layer is more than 20 percent of the Income attribute in the target layer. Use an Arcade expression such as $join[&quot;HealthSpending&quot;] &gt; $target[&quot;Income&quot;] * .2.</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
