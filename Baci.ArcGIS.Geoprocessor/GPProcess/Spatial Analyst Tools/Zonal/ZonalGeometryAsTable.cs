...
namespace Baci.ArcGIS.Geoprocessor.SpatialAnalystTools
{
	...
	public class ZonalGeometryAsTable : AbstractGPProcess
	{
		...
		/// <para>表的格式由输出位置和路径确定。默认情况下，输出为一张地理数据库表。如果路径不在地理数据库中，则格式将由表达式确定。如果扩展名为 .dbf，则将采用 dBASE 格式。如果未指定扩展名，则输出将为 INFO 表。请注意，不支持将 INFO 表作为 ArcGIS Pro 中的输入且无法显示 INFO 表。</para>
		/// </param>
