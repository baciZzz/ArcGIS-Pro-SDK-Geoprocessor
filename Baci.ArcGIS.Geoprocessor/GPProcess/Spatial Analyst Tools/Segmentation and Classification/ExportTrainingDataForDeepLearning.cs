...
namespace Baci.ArcGIS.Geoprocessor.SpatialAnalystTools
{
	...
	public class ExportTrainingDataForDeepLearning : AbstractGPProcess
	{
		...
		/// <para>矢量输入应当遵循使用训练样本管理器窗格生成的训练样本格式。 栅格输入应当遵循分类栅格工具生成的分类栅格格式。 栅格输入也可以来自分类栅格的文件夹。 输入表应当遵循通过训练样本管理器窗格中的深度学习标注对象工具生成的训练样本格式。 可以使用统计信息并遵循正确的训练样本格式生成最佳结果；但输入也可以是没有类值字段的点要素类，或不具有任何类信息的整型栅格。</para>
		/// </summary>
		...
		/// <para>指定输出元数据标注的格式。</para>
		/// <para>如果输入训练样本数据是诸如建筑物图层等的要素类图层或者标准分类训练样本文件，请使用 KITTI 标注 或 PASCAL Visual 对象类选项（Python 中的 KITTI_rectangles 或 PASCAL_VOC_rectangles）。 输出元数据是包含训练样本数据的 .txt 文件或 .xml 文件，其中训练样本数据包含在最小外接矩形中。 元数据文件的名称与输入源影像名称相匹配。 如果输入训练样本数据是类地图，请使用分类切片选项（Python 中的 Classified_Tiles）作为输出元数据格式。</para>
		/// <para>KITTI 标注—元数据遵循与卡尔斯鲁厄理工学院和丰田工业大学 (KITTI) 对象检测评估数据集相同的格式。 KITTI 数据集是一款视觉基准套件。 标注文件是纯文本文件。 所有的值（数值和字符串）均由空格分隔开，每行对应一个对象。此格式将用于对象检测。</para>
		/// <para>PASCAL 可视化对象类—元数据遵循与模式分析、统计建模和计算学习、可视化对象类 (PASCAL_VOC) 数据集相同的格式。 PASCAL VOC 数据集是用于对象类识别的标准化影像数据集。 标注文件为 XML 格式，包含有关影像名称、类值和边界框的信息。此格式将用于对象检测。 这是默认设置。</para>
		/// <para>已分类切片—每有一个输入影像片，就将输出一个分类影像片。 对于每个影像片，未使用其他元数据。 只有统计数据输出具有更多关于类的信息，如类名称、类值和输出统计数据。此格式主要用于像素分类。 当输出是来自两影像片中的一个分类影像片时，此格式也用于更改检测。</para>
		/// <para>RCNN 掩膜—输出将为在样本所在的区域上具有掩膜的影像片。 该模型将为影像中对象的每个实例生成边界框和分割掩膜。 此格式基于特征金字塔网络 (FPN) 和深度学习框架模型中的 ResNet101 核心支柱。此格式将用于对象检测。</para>
		/// <para>已标注切片—每个输出切片都将使用特定类进行标注。此格式将用于对象分类。</para>
		...
		/// <para>CycleGAN—输出将为不带标注的影像片。此格式用于影像转换技术 CycleGAN，该技术用于训练不重叠的影像。</para>
		...
		/// <para>用于描绘训练样本区域的，围绕每个训练样本的缓冲区的半径。 您可以借此从点创建圆形面训练样本。</para>
		/// <para>系统将使用输入要素类或分类栅格空间参考的线性单位。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPDouble()]
		...
		/// <para>影像片将在旋转角度为 0 的情况下生成，这意味着无旋转。 随后系统将以指定的角度旋转该片，以创建其他影像片。 系统将在多个影像片中以多个角度捕获相同的训练样本，以便用于数据增强。</para>
		/// <para>默认旋转角度为 0。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPDouble()]
