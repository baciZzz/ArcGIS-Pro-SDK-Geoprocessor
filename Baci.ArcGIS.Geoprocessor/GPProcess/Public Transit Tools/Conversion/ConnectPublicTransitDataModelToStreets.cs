...
namespace Baci.ArcGIS.Geoprocessor.PublicTransitTools
{
	...
	public class ConnectPublicTransitDataModelToStreets : AbstractGPProcess
	{
		...
		/// <para>A polyline feature class of streets to which transit stops and lines will connect. This streets feature class should be the same feature class you intend to use in the transit-enabled network dataset for modeling pedestrians walking along streets. The feature class does not need to be in the target feature dataset to run this tool; however, the feature class must be in the target feature dataset at the time you create the network dataset.</para>
		/// <para>The input streets features will be altered after running the tool. Vertices will be added at the locations where StopsOnStreets features intersect the streets. If you do not want the street data altered, make a copy of it before running this tool.</para>
		/// </param>
