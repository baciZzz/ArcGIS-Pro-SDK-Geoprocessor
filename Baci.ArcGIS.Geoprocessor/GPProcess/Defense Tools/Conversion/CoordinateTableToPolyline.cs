...
namespace Baci.ArcGIS.Geoprocessor.DefenseTools
{
	...
	public class CoordinateTableToPolyline : AbstractGPProcess
	{
		...
		/// <summary>
		/// <para>Tool Display Name : 坐标表转折线</para>
		/// </summary>
		public override string DisplayName() => "坐标表转折线";
		...
		/// <para>输入表字段中用于创建唯一折线的字段。 将针对每个唯一值创建一条折线。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[Field()]
		...
		/// <para>输入表中用于对折线折点进行排序的字段。 该字段必须为数值型。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[Field()]
