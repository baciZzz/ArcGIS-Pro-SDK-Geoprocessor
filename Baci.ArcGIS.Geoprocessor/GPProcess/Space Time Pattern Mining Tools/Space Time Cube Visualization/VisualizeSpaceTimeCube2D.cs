...
namespace Baci.ArcGIS.Geoprocessor.SpaceTimePatternMiningTools
{
	...
	public class VisualizeSpaceTimeCube2D : AbstractGPProcess
	{
		...
		/// <para>新兴时空热点分析结果—将显示指定立方体变量参数值的新兴时空热点分析的结果工具的结果。</para>
		/// <para>本地异常值分析结果—将显示指定立方体变量参数值的局部异常值分析工具的结果。</para>
		/// <para>局部异常值百分比—将显示每个位置的总异常值百分比。</para>
		/// <para>最近时间段内的局部异常值—将显示最近时间段内发生的异常值。</para>
		/// <para>时间序列聚类结果—将显示指定立方体变量参数值的时间序列聚类工具的结果。</para>
		...
		/// <para>预测结果—将显示指定分析变量参数值所使用的时间序列预测工具的结果。</para>
		/// <para>时间序列异常值结果—将显示时间序列预测工具中的异常值选项参数的结果。</para>
		/// <para>时间序列变化点—变化点检测工具的结果。 输出要素显示每个位置的变化点数量，以及显示原始时间序列、变化点和每条线段平均值或标准差的估算值的弹出图表。</para>
		/// </param>
