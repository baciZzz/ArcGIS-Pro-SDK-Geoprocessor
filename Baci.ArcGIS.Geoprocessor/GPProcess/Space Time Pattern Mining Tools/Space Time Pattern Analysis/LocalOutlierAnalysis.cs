...
namespace Baci.ArcGIS.Geoprocessor.SpaceTimePatternMiningTools
{
	...
	public class LocalOutlierAnalysis : AbstractGPProcess
	{
		...
		/// <para>要分析的 netCDF 立方体。 此文件必须具有 (.nc) 扩展名，并且必须使用通过聚合点创建时空立方体或通过已定义要素创建时空立方体工具进行创建。</para>
		/// </param>
		...
		/// <para>99—如果有 99 个排列，则可能的最小伪 p 值为 0.01，其他所有伪 p 值将是该值的偶数倍。</para>
		/// <para>199—如果有 199 个排列，则可能的最小伪 p 值为 0.005，其他所有伪 p 值将是该值的偶数倍。</para>
		/// <para>499—如果有 499 个排列，则可能的最小伪 p 值为 0.002，其他所有伪 p 值将是该值的偶数倍。</para>
		/// <para>999—如果有 999 个排列，则可能的最小伪 p 值为 0.001，其他所有伪 p 值将是该值的偶数倍。</para>
		/// <para>9999—如果有 9999 个排列，则可能的最小伪 p 值为 0.0001，其他所有伪 p 值将是该值的偶数倍。</para>
