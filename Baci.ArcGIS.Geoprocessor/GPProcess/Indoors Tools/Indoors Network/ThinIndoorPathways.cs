...
namespace Baci.ArcGIS.Geoprocessor.IndoorsTools
{
	...
	public class ThinIndoorPathways : AbstractGPProcess
	{
		...
		/// <para>输入面要素，表示一个或多个设施点中的一个或多个楼层。 在 ArcGIS Indoors 信息模型中，此项将为 Levels 图层。 该工具将仅处理这些要素表示的楼层。</para>
		/// </param>
		...
		/// <para>表示用于计算路径的位置的输入点或面要素。 该要素可以是符合 Indoors 模型的任何点或面要素，也可以是在图层的楼层属性中配置的或具有将要素与其所在楼层相关联的 LEVEL_ID 字段的任何楼层感知型点或面图层。</para>
		/// </param>
