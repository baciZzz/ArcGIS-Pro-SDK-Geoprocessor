...
namespace Baci.ArcGIS.Geoprocessor.ImageAnalystTools
{
	...
	public class FocalStatistics : AbstractGPProcess
	{
		...
		/// <summary>
		/// <para>Tool Excute Name : ia.FocalStatistics</para>
		/// </summary>
		public override string ExcuteName() => "ia.FocalStatistics";
		...
		/// <para>百分比数—将计算邻域内像元的百分比数。 默认情况下将计算 90% 百分比数。 您可以使用百分比值参数来指定其他值（从 0 到 100）。</para>
		...
		/// <para>选中 - 当邻域中存在 NoData 值时，将忽略此 NoData 值。 将仅使用邻域内具有数据值的像元来确定输出值。 这意味着如果处理像元值为 NoData，则一旦选中该参数，处理像元即可在输出栅格中接收值，前提是该邻域内至少有一个像元具有有效值。 这是默认设置。</para>
		/// <para>未选中 - 如果邻域内有任意像元的值是 NoData（包括处理像元），则处理像元的输出将为 NoData。 如果未选中此参数，存在 NoData 值表明确定邻域的统计值所需要的信息不足。</para>
		...
		/// <para>要计算的百分比数。 对于 90%，其默认值为 90。</para>
		/// <para>取值范围为 0 到 100。 0% 基本上等同于“最小值”统计数据，而 100% 则等同于“最大值”。 值 50 所生成的结果基本等同于“中值”统计数据的结果。</para>
		/// <para>此选项仅在将统计类型参数设置为百分比数后受支持。 如果已指定任何其他统计类型，则将忽略此参数。&lt;bold/&gt;</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPDouble()]
		[GPNumericDomain()]
