...
namespace Baci.ArcGIS.Geoprocessor.ImageAnalystTools
{
	...
	public class TrainDeepLearningModel : AbstractGPProcess
	{
		...
		/// <para>要训练模型，输入图像必须是具有三个波段的 8 位栅格。</para>
		/// </param>
		...
		/// <para>应用于将训练模型的最大新纪元数。 最大新纪元值为 1 意味着数据集将通过神经网络向前和向后传递一次。 默认值为 20。</para>
		/// </summary>
		...
		/// <para>一次需要处理以便用于训练的训练样本数。 默认值为 2。</para>
		/// <para>如果您具有功能强大的 GPU，则可将此数字增加到 8、16、32 或 64。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPLong()]
		[Category("Model Parameters")]
		...
		/// <para>当您将单帧检测器选为模型类型参数值时，模型参数参数将使用以下参数进行填充：</para>
		...
		/// <para>当您选择像素分类模型（例如，金字塔场景解析网络、U-Net 或 DeepLabv3）作为模型类型参数值时，模型参数参数将使用以下参数进行填充：</para>
		...
		/// <para>当选择 RetinaNet 作为模型类型参数值时，模型参数参数将使用以下参数进行填充：</para>
		/// <para>比例 - 每个像元将按比例进行扩大或缩小的比例级数。 默认值为 [1, 0.8, 0.63]。</para>
		/// <para>比率 - 锚点框的纵横比。 默认值为 0.5,1,2。</para>
		/// <para>当您将多任务道路提取器或 ConnectNet 选为模型类型参数值时，模型参数参数将使用以下参数进行填充：</para>
		...
		/// <para>当选择图像标题生成器作为模型类型参数值时，模型参数参数将使用以下参数进行填充：decode_params 由以下六个参数组成：</para>
		...
		/// <para>当您将变化检测器选为模型类型参数值时，模型参数参数将使用以下参数进行填充：</para>
		...
		/// <para>选中 - 将实施提前停止，当模型不再改进时，无论所指定的最大新纪元参数值是什么，模型训练都将停止。 这是默认设置。</para>
		/// <para>未选中 - 不会实施提前停止，模型训练将一直持续，直至达到最大新纪元参数值为止。</para>
