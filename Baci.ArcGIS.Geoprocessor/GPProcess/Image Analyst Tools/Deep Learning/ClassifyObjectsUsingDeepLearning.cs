...
namespace Baci.ArcGIS.Geoprocessor.ImageAnalystTools
{
	...
	public class ClassifyObjectsUsingDeepLearning : AbstractGPProcess
	{
		...
		/// <para>将用于检测对象的输入图像。 输入可以是镶嵌数据集、影像服务或影像文件夹中的单个栅格或多个栅格。 还支持具有影像附件的要素类。</para>
		/// </param>
		...
		/// <para>如果未指定输入要素类，将假设每个输入影像包含单个待分类对象。 如果一个或多个输入影像使用空间参考，则工具的输出为要素类，其中每个影像的范围将用作每个标注要素类的边界几何。 如果一个或多个输入影像没有使用空间参考，则工具的输出为包含影像 ID 值和每个影像类标注的表。</para>
		/// </summary>
		...
		/// <para>如果未指定字段名称，则会在输出要素类中生成一个 ClassLabel 字段。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
		...
		/// <summary>
		/// <para>Caption</para>
		/// <para>包含输出要素类中文本或标题的字段名称。 仅当使用“影像描述生成器”模型时才支持此参数。</para>
		/// <para>如果未指定字段名称，则会在输出要素类中生成一个 Caption 字段。</para>
		/// <para>此参数不会出现在地理处理窗格中。 要更改默认字段名称，请使用类标注字段参数。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
