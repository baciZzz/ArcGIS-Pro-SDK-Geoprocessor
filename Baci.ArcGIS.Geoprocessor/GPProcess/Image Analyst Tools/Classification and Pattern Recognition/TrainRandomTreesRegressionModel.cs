...
namespace Baci.ArcGIS.Geoprocessor.ImageAnalystTools
{
	...
	public class TrainRandomTreesRegressionModel : AbstractGPProcess
	{
		...
		/// <para>包含目标变量（因变量）数据的栅格或点要素类。</para>
		/// </param>
		...
		/// <para>将用于错误检查的测试点的百分比。 此工具用于检查三种类型的错误：训练点错误、测试点错误和测试位置点错误。 默认值为 10。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPDouble()]
		[Category("Training Options")]
		...
		/// <para>PDF 或 HTML 格式的输出散点图。 输出将包含训练数据、测试数据和位置测试数据的散点图。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[DEFile()]
		[GPCompositeDomain()]
		...
		/// <para>输出要素类将包含训练点、测试点和位置测试点的目标值和预测值。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[DEFeatureClass()]
		[Category("Additional Outputs")]
