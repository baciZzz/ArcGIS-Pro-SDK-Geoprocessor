...
namespace Baci.ArcGIS.Geoprocessor.SpatialStatisticsTools
{
	...
	public class DensityBasedClustering : AbstractGPProcess
	{
		...
		/// <para>要考虑的最大距离。</para>
		/// <para>对于 cluster_method 参数的 DBSCAN 选项，必须在聚类成员资格的此距离内找到 min_features_cluster 参数值。 将至少按此距离来分隔单个聚类。 如果点与聚类中下一最近点的距离大于此距离，则不会将该点包括在聚类中。</para>
		/// <para>对于 cluster_method 参数的 OPTICS 选项，此参数是可选的，并且可用作创建可达图时的最大搜索距离。 对于 OPTICS，结合 cluster_sensitivity 参数值的可达图可以确定聚类成员资格。 如果未指定距离，则工具将搜索所有距离，这会增加处理时间。</para>
		/// <para>如果留空，则使用的默认距离将为数据集中找到的最高核心距离，排除前 1% 的核心距离（即最极端的核心距离）。 如果提供 time_field 参数值，必须提供搜索距离且不得包括默认值。</para>
