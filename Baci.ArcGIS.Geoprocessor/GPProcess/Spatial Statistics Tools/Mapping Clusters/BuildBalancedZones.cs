...
namespace Baci.ArcGIS.Geoprocessor.SpatialStatisticsTools
{
	...
	/// <para>构建平衡区域</para>
	/// <para>使用基于您指定标准的遗传增长算法在研究区域中创建空间连续区域。</para>
	/// </summary>
	public class BuildBalancedZones : AbstractGPProcess
	{
		...
		/// <para>指定将用于增长每个区域的方法。区域将增长，直到满足所有指定条件为止。</para>
		/// <para>属性目标—将根据一个或多个变量的目标值创建区域。如果选择此选项，则必须在具有目标的区域构建标准参数中指定所需的每个属性的总和；同时每个区域都会增长，直到属性的总和超过这些值为止。例如，您可以使用此选项创建每个区域中至少具有 100,000 个居民和 20,000 个家庭住宅的区域。</para>
		/// <para>区域数和属性目标—系统将创建指定数量的区域，同时使每个区域内的属性总和大致相等。如果选择此选项，则必须在目标区域数参数中指定所需的区域数。每个区域内的属性总和均等于用属性总和除以区域数得到的商。</para>
		/// <para>所定义区域数—将创建指定数量的区域，其中每个区域都由数量大致相同的输入要素组成。如果选择此选项，则必须在目标区域数参数中指定所需的区域数。</para>
		...
		/// <para>指定要考虑的变量、其目标值以及权重（可选）。默认权重设置为 1，除非另行更改，否则每个变量的权重均相同。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPValueTable()]
		[GPCompositeDomain()]
		...
		/// <para>指定将被考虑的变量以及权重（可选）。默认权重设置为 1，除非另行更改，否则每个变量的权重均相同。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPValueTable()]
		[GPCompositeDomain()]
		...
		/// <para>指定在区域增长时如何定义相邻要素。区域只能增长为区域中已有的至少一个要素的相邻要素的新要素。如果输入要素为面，则默认空间约束为邻接边拐角。如果输入要素为点，则默认空间约束为修剪型 Delaunay 三角测量。</para>
		...
		/// <para>指定将创建的区域的所需的特征。</para>
		/// <para>相等面积— 将创建总面积尽可能相似的区域。</para>
		/// <para>紧密度—将使用更紧密（紧凑）的要素创建区域。</para>
		...
		/// <para>指定在选择最终区域时要考虑的属性和统计数据。您可以选择根据属性的总和、平均值、中位数或方差对属性进行同质化。例如，如果您要基于房屋价格创建区域并希望平衡每个区域内的平均总收入，则最好使用跨区域平均收入最相等的解决方案。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPValueTable()]
		[GPCompositeDomain()]
		...
		/// <para>区域搜索过程重复的次数。对于较大的数据集，建议增加数量以找到最佳解决方案。默认值为 50 代。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPLong()]
		...
		/// <para>如果指定，将创建一个表，其中包含每一代中找到的最佳解决方案的总体适用性分数以及各个区域约束的适用性分数。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[DETable()]
		[Category("Advanced Parameters")]
