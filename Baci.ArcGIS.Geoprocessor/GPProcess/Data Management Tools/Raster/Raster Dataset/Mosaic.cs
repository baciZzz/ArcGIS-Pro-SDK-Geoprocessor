...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	public class Mosaic : AbstractGPProcess
	{
		...
		/// <para>输入栅格将被添加到的栅格。 此栅格数据集必须已经存在。 默认情况下，目标栅格被视为输入栅格数据集列表中的第一个栅格。 使用创建栅格数据集工具可创建空栅格。</para>
		/// </param>
		...
		/// <para>对于基于文件的栅格和地理数据库栅格，要忽略背景值，必须将其设置为与 NoData 相同的值。 企业级地理数据库栅格无需经过此额外步骤即可忽略背景值。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPDouble()]
		...
		/// <para>指定将应用于栅格的色彩匹配方法。</para>
		/// <para>无—镶嵌栅格数据集时将不使用颜色匹配操作。</para>
