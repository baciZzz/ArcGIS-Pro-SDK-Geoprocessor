...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	/// <para>计算统计数据</para>
	/// <para>用于计算栅格数据集或镶嵌数据集的统计数据。</para>
	/// </summary>
	public class CalculateStatistics : AbstractGPProcess
	{
		...
		/// <summary>
		/// <para>Tool Display Name : 计算统计数据</para>
		/// </summary>
		public override string DisplayName() => "计算统计数据";
		...
		/// <para>默认情况下没有值，或者是上次使用的忽略值。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPMultiValue()]
		...
		/// <para>指定是在缺少统计数据的位置进行计算，还是重新计算全部统计数据（即使已经存在仍重新计算）。</para>
		/// <para>未选中 - 即使统计数据已经存在仍要重新计算；因此，现有统计数据将被覆盖。 这是默认设置。</para>
		...
		/// <para>数据集中开始计算统计数据的区域，这样，可避免从整个数据集生成统计数据。可以浏览至要素类，或者在显示画面上创建面图形。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
