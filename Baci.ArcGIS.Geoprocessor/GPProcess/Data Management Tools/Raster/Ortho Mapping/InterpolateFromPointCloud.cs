...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	/// <para>基于点云进行插值</para>
	/// <para>可使用提供的插值方法之一，基于点云对数字地形模型 (DTM) 或数字表面模型 (DSM) 进行插值。</para>
	/// </summary>
	public class InterpolateFromPointCloud : AbstractGPProcess
	{
		...
		/// <para>可以采用大多数可写栅格格式创建输出，例如 .tif、.crf 或 .img。</para>
		/// </param>
		/// <param name="CellSize">
		/// <para>Cellsize</para>
		...
		/// <para>指定用于从点云对输出栅格数据集进行插值的方法。</para>
		/// <para>TIN 线性插值法— 该方法亦称作不规则三角网 (TIN) 线性插值法，适用于不规则分布的稀疏点，例如从区域网平差计算中得到的解决方案点。</para>
		/// <para>TIN 自然邻域插值法—这与三角测量类似，但是生成的表面更为平滑且运算量更大。</para>
		/// <para>反距离权重平均插值法—此方法适用于规则分布的密集点，例如通过生成点云工具得到的点云 LAS 文件。IDW 搜索半径将根据平均点密度自动进行计算。</para>
		/// <para><see cref="InterpolationMethodEnum"/></para>
		/// </param>
		/// <param name="SmoothMethod">
		/// <para>Smoothing Method</para>
		/// <para>指定用于平滑输出栅格数据集的滤波器。</para>
		...
		/// <para>指定将创建数字地形模型还是数字表面模型。</para>
		/// <para>数字地形模型—将通过仅插值地面点来创建数字地形模型。</para>
		/// <para>数字表面模型—将通过插值所有点来创建数字表面模型。</para>
		...
		/// <para>用于填充 NoData 区域的 DEM 栅格输入。NoData 区域可以存在于像素未从输入获得足够生成任何值所需信息的位置。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPComposite()]
