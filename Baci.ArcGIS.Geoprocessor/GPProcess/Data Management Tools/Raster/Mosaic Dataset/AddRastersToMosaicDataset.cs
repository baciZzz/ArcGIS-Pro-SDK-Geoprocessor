...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	public class AddRastersToMosaicDataset : AbstractGPProcess
	{
		...
		/// <para>文件—一个或多个存储于磁盘的文件夹、影像服务定义文件 (.ISDef) 或栅格处理定义文件 (.RPDef) 中的栅格数据集将被用作输入。文件列表将忽略与添加的栅格类型不对应的文件。此选项不得与栅格数据集文件格式（如 TIFF 或 MrSID 文件）配合使用，请改为使用数据集输入类型。</para>
		...
		/// <para>排除副本—不会添加重复的栅格。</para>
		/// <para>覆盖副本—重复的栅格将覆盖现有的栅格。</para>
		...
		/// <para>选中 - 加载数据时，将输入数据的坐标系参数中指定的坐标系用于各个栅格数据集。</para>
