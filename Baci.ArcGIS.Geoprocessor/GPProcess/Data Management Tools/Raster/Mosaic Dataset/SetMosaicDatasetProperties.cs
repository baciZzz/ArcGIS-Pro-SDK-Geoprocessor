...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	public class SetMosaicDatasetProperties : AbstractGPProcess
	{
		...
		/// <para>三次卷积—通过根据周围的 16 像素拟合平滑曲线来计算每个像素的值。 此技术将生成平滑影像，但可创建超出源数据范围的值。 适用于连续数据。</para>
		...
		/// <para>未选中 - 不显示包含 NoData 值的像素。 您可能会注意到性能有所提高；但是，如果影像确实包含 NoData 值，则它们将在镶嵌数据集中显示为孔。</para>
		...
		/// <para>指定将用于时间间隔的测量单位。</para>
		/// <para>无—不存在时间单位或时间单位未知。</para>
		/// <para>毫秒—时间单位将为毫秒。</para>
		/// <para>秒—时间单位将为秒。</para>
		/// <para>分—时间单位将为分钟。</para>
		/// <para>小时—时间单位将为小时。</para>
		/// <para>天—时间单位将为天。</para>
		/// <para>周—时间单位将为周。</para>
		/// <para>月—时间单位将为月。</para>
		/// <para>年—时间单位将为年。</para>
		/// <para>十年—时间单位将为十年。</para>
		/// <para>世纪—时间单位将为百年。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
		[GPCodedValueDomain()]
