...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	public class AlterDomain : AbstractGPProcess
	{
		...
		/// <para>包含将更改的属性域的地理数据库。</para>
		/// </param>
		/// <param name="DomainName">
		/// <para>Domain Name</para>
		/// <para>将更改的属性域的名称。</para>
		/// </param>
		...
		/// <para>指定将用于属性域的分割策略。 分割要素时，属性值的行为受控于它的分割策略。</para>
		/// <para>使用属性的默认值—两个结果要素的属性将为给定要素类或子类型的默认属性值。</para>
		/// <para>复制属性值—两个结果要素的属性将为原始对象的属性值副本。</para>
		/// <para>使用几何比—结果要素的属性将是原始要素值的比率。 该比率取决于原始几何的分割比例。 如果几何被分割成相等的两部分，则每个新要素的属性值将是原始对象属性值的一半。 此选项仅适用于范围属性域。</para>
		...
		/// <para>指定将用于属性域的合并策略。 在将两个要素合并为一个要素时，合并策略将控制新要素的属性值。 由于编码值属性域可能仅使用默认合并策略，因此此参数仅适用于范围属性域。</para>
		/// <para>使用属性的默认值—结果要素的属性将为给定要素类或子类型的默认属性值。 此选项仅适用于非数字字段和编码值属性域。</para>
		/// <para>值的总和—结果要素的属性将基于原始要素属性值的总和。 此选项仅适用于范围属性域。</para>
		/// <para>面积加权平均值—结果要素的属性将使用原始要素属性值的加权平均值。 此平均值取决于原始要素的几何。 此选项仅适用于范围属性域。</para>
		...
		/// <para>属性域所有权将转移到的数据库用户的名称。 确保数据库中存在新的属性域所有者；该工具不会检查指定所有者名称的有效性。 此参数不适用于在文件地理数据库中创建的属性域。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
