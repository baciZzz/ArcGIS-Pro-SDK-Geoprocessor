...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	public class CreateDomain : AbstractGPProcess
	{
		...
		/// <para>文本—字段类型将为文本型。 文本字段支持字符串。</para>
		/// <para>浮点型（32 位浮点型）—字段类型将为浮点型。 浮点型字段支持介于 -3.4E38 和 1.2E38 之间的小数。</para>
		/// <para>双精度型（64 位浮点型）—字段类型将为双精度型。 双精度字段支持介于 -2.2E308 和 1.8E308 之间的小数。</para>
		/// <para>短整型（16 位整型）—字段类型将为短整型。 短整形字段支持介于 -32,768 和 32,767 之间的整数。</para>
		/// <para>长整型（32 位整数）—字段类型将为长整型。 长整型字段支持介于 2,147,483,648 和 2,147,483,647 之间的整数。</para>
		/// <para>日期—字段类型将为日期型。 日期字段支持日期和时间值。</para>
