...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	public class CreateDatabaseConnection : AbstractGPProcess
	{
		...
		/// <para>Oracle—连接到 Oracle、Amazon Relational Database Service (RDS) for Oracle 或 Autonomous Transaction Processing database in Oracle Cloud。</para>
		...
		/// <para>SQL Server—连接到 Microsoft SQL Server、Microsoft Azure SQL Database、Microsoft Azure SQL Managed Instance 或 Amazon Relational Database Service (RDS) for SQL Server。</para>
		...
		/// <para>从数据库平台下拉列表中选择的值表示要连接的数据库的类型。 根据所选择的连接类型，可为实例参数提供不同的信息。</para>
		...
		/// <para>选中 - 用户名和密码将保存在连接文件中。 这是默认设置。 如果 ArcGIS 服务可通过您所创建的连接文件来对地理数据库进行访问，则必须保存用户名和密码。</para>
