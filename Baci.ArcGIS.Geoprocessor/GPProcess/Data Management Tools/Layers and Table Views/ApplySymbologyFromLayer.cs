...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	public class ApplySymbologyFromLayer : AbstractGPProcess
	{
		...
		/// <para>The layer containing the symbology that will be applied to the input layer. Both .lyrx and .lyr files are supported.</para>
		/// </param>
		...
		/// <para>Field type—The field type: symbology value, normalization, or other type.</para>
		...
		/// <para>Default—Symbology ranges will be updated, except when the input layer is empty; when the symbology layer uses class breaks (for example, graduated colors or graduated symbols) and the classification method is manual or defined interval; or when the symbology layer uses unique values and the Show all other values option is checked.</para>
