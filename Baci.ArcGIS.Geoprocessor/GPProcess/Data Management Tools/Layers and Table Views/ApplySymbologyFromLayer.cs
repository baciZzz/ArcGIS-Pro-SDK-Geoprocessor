...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	public class ApplySymbologyFromLayer : AbstractGPProcess
	{
		...
		/// <para>包含将应用于输入图层的符号系统的图层。 支持 .lyrx 和 .lyr 文件。</para>
		/// </param>
		...
		/// <para>字段类型 - 字段类型：符号系统值、归一化或其他类型。</para>
		...
		/// <para>默认值—除了以下情形之外，将对符号系统范围进行更新：当输入图层为空时；当符号系统图层使用分类间隔（例如，分级色彩或分级符号），并且分类方法为手动间隔或定义间隔时；或者，当符号系统图层使用唯一值，并且选中显示所有其他值选项时。</para>
