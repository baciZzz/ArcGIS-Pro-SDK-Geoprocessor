...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	public class EnableFeatureBinning : AbstractGPProcess
	{
		...
		/// <para>要启用要素图格化的要素类。 仅支持在企业级地理数据库、数据库或云数据仓库中存储的点和多点要素类。 数据无法启用版本化或存档。</para>
		/// </param>
		...
		/// <summary>
		/// <para>Tool Display Name : 启用要素图格化</para>
		/// </summary>
		public override string DisplayName() => "启用要素图格化";
		...
		/// <summary>
		/// <para>Bin Type</para>
		/// <para>指定要启用的图格化类型。 如果正在使用 SAP HANA 数据，则仅支持正方形、平六边形和尖六边形选项。 如果正在使用 Snowflake 或 Redshift 数据，则仅支持 Geohash 选项。</para>
		/// <para>平六边形—将启用平面六边形图格化方案，也称为平面几何六边形或平面六边形图格化。 切片是六边形方向在顶部具有六边形平边的六边形镶嵌。 这是 Microsoft SQL Server、Oracle、PostgreSQL 和 BigQuery 数据的默认值。</para>
		/// <para>尖六边形—将启用尖六边形图格化方案，也称为尖几何六边形或尖六边形图格化。 切片是六边形方向在顶部具有六边形点的六边形镶嵌。</para>
		/// <para>方形—将启用正方形图格化方案，也称为几何正方形或正方形图格化。 切片是矩形的细分。这是 Db2 和 SAP HANA 数据的默认值。</para>
		/// <para>Geohash—将启用 Geohash 图格化方案，其中切片是矩形的细分。 由于 Geohash 图格始终使用 WGS 1984 地理坐标系（GCS WGS 1984、EPSG WKID 4326），因此无法为 Geohash 图格指定图格坐标系。 这是 Snowflake 或 Redshift 中数据的默认选项和唯一选项。</para>
		...
		/// <para>此参数不适用于 BigQuery、Redshift 或 Snowflake。 对于这些平台，将使用输入要素类的坐标系。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPMultiValue()]
		...
		/// <para>选中 - 将生成已聚合结果的静态缓存。 要提高性能，建议使用此选项。 但是，对基础数据所做的更改将不会在缓存中更新，除非运行管理要素图格缓存工具。</para>
		/// <para>默认情况下，将为 IBM Db2、Microsoft SQL Server、Oracle 和 PostgreSQL 中的数据生成静态缓存。</para>
		/// <para>要为 PostgreSQL 中使用 PostGIS 空间类型的要素类生成静态缓存，必须在数据库中安装 GDAL 库。</para>
		/// <para>将始终为 BigQuery、Redshift 和 Snowflake 中的数据生成静态缓存。</para>
