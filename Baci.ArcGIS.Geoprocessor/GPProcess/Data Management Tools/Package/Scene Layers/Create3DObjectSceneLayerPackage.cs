...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	/// <para>创建 3D 对象场景图层包</para>
	/// <para>用于从多面体或 3D 对象要素图层输入创建场景图层包（.slpk 文件）。</para>
	/// </summary>
	public class Create3DObjectSceneLayerPackage : AbstractGPProcess
	{
		...
		/// <summary>
		/// <para>Tool Display Name : 创建 3D 对象场景图层包</para>
		/// </summary>
		public override string DisplayName() => "创建 3D 对象场景图层包";
		...
		/// <para>指定根据使用场景图层包的目标平台优化的纹理。可能需要大量时间来处理包括 ETC2 的优化。 要获得最快结果，请使用桌面或无选项。</para>
		/// <para>全部—所有用于桌面、web 和移动平台的纹理格式都将进行优化，包括 JPEG、DXT 和 ETC2。</para>
		...
		/// <para>移动—支持 Android 和 iOS 的纹理将进行优化，包括 JPEG 和 ETC2，可用于 ArcGIS Runtime 移动应用程序。</para>
