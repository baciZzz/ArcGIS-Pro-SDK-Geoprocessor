...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	/// <para>创建点云场景图层包</para>
	/// <para>从 LAS、zLAS、LAZ 或 LAS 数据集输入创建点云场景图层包（.slpk 文件）。</para>
	/// </summary>
	public class CreatePointCloudSceneLayerPackage : AbstractGPProcess
	{
		...
		/// <para>激光雷达数据（LAS、zLAS、LAZ 或 LAS 数据集），可用于创建场景图层包。也可以通过选择包含所需文件的父文件夹来指定激光雷达数据。</para>
		/// </param>
		/// <param name="OutSlpk">
		...
		/// <summary>
		/// <para>Tool Display Name : 创建点云场景图层包</para>
		/// </summary>
		public override string DisplayName() => "创建点云场景图层包";
		...
		/// <para>强度— 每个激光雷达点的激光脉冲的回波强度。</para>
		/// <para>RGB—针对每个激光雷达点采集的 RGB 影像信息。</para>
		/// <para>LAS 标记—分类和扫描方向标记。</para>
		/// <para>分类代码—分类代码值。</para>
		/// <para>返回值—激光雷达脉冲的离散回波编号。</para>
		/// <para>用户数据—可自定义属性的取值范围是 0 到 255。</para>
		/// <para>点源 ID—对于航空激光雷达，此值通常用于标识采集了给定激光雷达点的飞行路径。</para>
		/// <para>GPS 时间— 从飞机发射激光点的 GPS 时间戳。此时间以 GPS 一周的秒数表示，其中时间戳介于 0 和 604800 之间，并在星期日的午夜重置。</para>
		/// <para>扫描角度—给定激光雷达点的激光扫描仪的角度方向。值的范围从 -90 到 90。</para>
		/// <para>近红外—针对每个激光雷达点采集的近红外记录。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPMultiValue()]
		...
		/// <para>容许最大 x,y 误差。容差越大，数据压缩越好，并且数据传输效率越高。值以米为单位表示。默认值为 0.01。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPDouble()]
		...
		/// <para>容许最大 z 误差。容差越大，数据压缩越好，并且数据传输效率越高。值以米为单位表示。默认值为 0.01。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPDouble()]
		...
		/// <para>输入 LAZ 文件的坐标系。此参数仅用于标头中不包含空间参考信息且不具有 .prj 文件的 LAZ 文件。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPCoordinateSystem()]
		...
		/// <para>1.x—在所有 ArcGIS 客户端中均受支持。</para>
		/// <para>2.x—在 ArcGIS Pro 2.1.2 或更高版本中受支持，并且可以发布到 ArcGIS Online 以及 ArcGIS 10.6.1 或更高版本。这是默认设置。</para>
