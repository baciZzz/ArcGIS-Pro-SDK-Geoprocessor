...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	public class CreateMobileMapPackage : AbstractGPProcess
	{
		...
		/// <para>定义感兴趣区的面图层。仅与感兴趣区相交的要素才会包括在移动地图包中。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPFeatureLayer()]
		...
		/// <para>确定是否将输出要素裁剪为给定的感兴趣区或范围。</para>
		...
		/// <para>确定移动地图是否可由所有人使用。</para>
		/// <para>选中 - 允许具有包访问权限的所有人使用移动地图，无需使用 Esri 授权用户帐户进行登录。</para>
		/// <para>未选中 - 具有包访问权限的所有人必须使用授权用户帐户进行登录，才能使用移动地图。这是默认设置。</para>
		...
		/// <para>选中 - 已在移动地图包上启用超时。</para>
		/// <para>未选中 - 已在移动地图包上禁用超时。这是默认设置。</para>
		/// <para>此可选参数仅适用于 Publisher 扩展模块。</para>
		...
		/// <para>指定已到期移动地图包的访问类型。</para>
		/// <para>允许打开—警告包用户地图已到期但允许打开。这是默认设置。</para>
		/// <para>不允许打开—警告包用户地图已到期且不允许打开。</para>
		/// <para>此可选参数仅适用于 Publisher 扩展模块。</para>
