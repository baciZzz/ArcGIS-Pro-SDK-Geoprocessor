...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	/// <para>添加空间连接</para>
	/// <para>根据空间关系将一个要素类的属性连接到另一个要素类的属性。 将连接目标要素和来自连接要素的连接属性。 此连接是临时连接。</para>
	/// </summary>
	public class AddSpatialJoin : AbstractGPProcess
	{
		...
		/// <para>目标要素的属性和连接要素的属性将连接到目标要素图层。 但是，可以使用字段映射参数中定义属性的子集。</para>
		/// </param>
		...
		/// <summary>
		/// <para>Tool Display Name : 添加空间连接</para>
		/// </summary>
		public override string DisplayName() => "添加空间连接";
		...
		/// <para>一对一连接—如果找到与同一目标要素存在相同空间关系的多个连接要素，将使用字段映射合并规则对多个连接要素中的属性进行聚合。 例如，如果在两个独立的面连接要素中找到了同一个点目标要素，将对这两个面的属性进行聚合，然后将其传递到输出点要素类。 如果一个面要素的属性值为 3，另一个面要素的属性值为 7，且指定了“总和”合并规则，则聚合值将为 10。 这是默认设置。</para>
		/// <para>一对多连接—如果找到多个与同一目标要素存在相同空间关系的连接要素，则连接将包含目标要素的多个副本（记录）。 例如，如果在两个独立的面连接要素中找到了同一个点目标要素，则输入要素图层将包含目标要素的两个副本：分别包含两个面的属性记录。</para>
		...
		/// <para>指定是仅保留与连接要素（称为内部连接）具有空间关系的目标要素，还是保留所有目标要素，即使与连接要素没有空间关系（称为外部连接）。</para>
		/// <para>选中 - 将保留目标要素图层中的所有要素。 这是默认设置。</para>
		/// <para>未选中 - 仅保留目标要素图层中与连接要素具有空间关系的要素。</para>
		...
		/// <para>最大重叠—连接要素中的特征将与重叠最大的目标要素匹配。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
		[GPCodedValueDomain()]
		...
		/// <para>添加到连接中的字段的名称，其中包含目标要素和最近连接要素之间的距离。 仅当已指定空间关系时（匹配选项设置为最近或最近测地线），此选项才有效。 如果在搜索半径内没有任何匹配的要素，则此字段的值为 -1。 如果未指定字段名称，则不会将该字段添加到连接中。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPString()]
