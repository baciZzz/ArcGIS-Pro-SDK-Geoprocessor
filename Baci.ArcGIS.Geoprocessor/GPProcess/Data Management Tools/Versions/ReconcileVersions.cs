...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	public class ReconcileVersions : AbstractGPProcess
	{
		...
		/// <para>指定发生冲突所需的条件是由对象（行）还是由属性（列）定义。</para>
		/// <para>按对象定义的冲突（按行）—协调期间父版本和子版本中的相同行或要素发生任何更改。这是默认设置。</para>
		/// <para>按属性定义的冲突（按列）—协调期间只有父版本和子版本中的相同行或要素的同一属性（列）发生的更改会被标记为冲突。协调期间不同属性所发生的更改不会被视为冲突。</para>
		...
		/// <para>写入日志文件的名称和位置。日志文件为包含地理处理消息内容的 ASCII 文件。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[DEFile()]
		...
		/// <para>如果协调过程开始之前存在未经审核的冲突，则指定是否将继续进行协调。如果选择继续，则执行工具时，之前会话中未解决的冲突将会丢失。此参数仅适用于分支版本化。</para>
		/// <para>选中 - 如果未解决的冲突尚未经过审核，则将继续协调过程。这是默认设置。</para>
		/// <para>未选中 - 如果检测到未经审核的未解决冲突，则不继续进行协调过程。</para>
		...
		/// <para>指定协调是否将包括检出复本版本。如果要将检出复本创建为地理数据库复制工作流的一部分，则系统会在地理数据库中创建相关联的版本。您可通过此选项在要协调的版本列表中包括或移除这些类型的版本。此参数不适用于分支版本化。</para>
