...
namespace Baci.ArcGIS.Geoprocessor.DataManagementTools
{
	...
	/// <para>管理切片缓存</para>
	/// <para>创建切片缓存或在现有的切片缓存中更新切片。 此工具用于创建新切片、替换缺失切片、覆盖过时切片以及删除切片。</para>
	/// </summary>
	public class ManageTileCache : AbstractGPProcess
	{
		...
		/// <para>创建缓存数据集所在的文件夹或现有切片缓存的路径。</para>
		/// </param>
		...
		/// <para>如果已在管理模式参数中指定删除切片，则不需要此参数。</para>
		...
		/// <para>ArcGIS Online 方案—使用默认 ArcGIS Online 切片方案。</para>
		/// <para>导入方案—导入现有的切片方案。</para>
		/// <para>高程切片方案—使用此高程服务切片方案。</para>
		/// <para>WGS84　版本　2　切片方案—使用此基于　WGS84 版本　2　的切片方案。</para>
		/// <para>WGS84　版本　2　高程切片方案—使用此基于 WGS84 版本 2 的切片方案来构建高程数据的切片缓存。</para>
		...
		/// <para>现有方案文件 (.xml) 的路径或从现有影像服务或地图服务中导入的路径。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPComposite()]
		...
		/// <para>一系列比例级别，运行此工具时将在这些比例级别上创建或删除切片，具体取决于管理模式参数的值。 像素大小是根据切片方案的空间参考表示的。</para>
		/// <para>默认情况下，将仅选中最小缓存比例和最大缓存比例之间的比例。</para>
		/// <para>更改最小缓存比例或最大缓存比例参数的值将选中或取消选中相应的比例值。</para>
		/// <para>生成缓存时将忽略选中的比例以及不在最小缓存比例或最大缓存比例范围之内的比例。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPMultiValue()]
		...
		/// <para>该参数用于为形状不规则的区域管理切片。 它对您要对某些区域进行预缓存或让较少访问的区域保持未缓存的状态等情形也同样有用。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		...
		/// <para>想要创建切片的最小比例。 这不必是您的切片方案中的最小比例。 由最小缓存比例确定生成缓存时将使用哪个比例。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPDouble()]
		[GPCodedValueDomain()]
		...
		/// <para>想要创建切片的最大比例。 这不必是您的切片方案中的最大比例。 由最大缓存比例确定生成缓存时将使用哪个比例。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPDouble()]
		[GPCodedValueDomain()]
