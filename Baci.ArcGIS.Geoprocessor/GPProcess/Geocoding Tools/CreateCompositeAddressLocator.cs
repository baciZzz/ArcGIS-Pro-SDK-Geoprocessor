...
namespace Baci.ArcGIS.Geoprocessor.GeocodingTools
{
	...
	/// <para>创建复合地址定位器</para>
	/// <para>创建复合地址定位器。 一个复合定位器由两个或更多的定位器组成，这些定位器允许使用多个定位器来匹配地址。</para>
	/// </summary>
	public class CreateCompositeAddressLocator : AbstractGPProcess
	{
		...
		/// <para>将用于创建复合定位器的定位器。 参与定位器的顺序决定了候选项的搜索方式以及地点或地址的匹配方式。 对单个地点或地址进行地理编码时，除非为定位器指定了选择条件，否则将根据所有参与地点或地址定位器来匹配地址。 系统将根据参与定位器的列出顺序来显示找到的所有候选项。 对地点或地址表进行地理编码时，地点或地址将自动与在前几个参与定位器中找到的第一个最佳候选项匹配。 如果地点或地址匹配失败，它将回退到列表中后面的定位器。</para>
		/// <para>每个参与的定位器都需要一个参考名称。 该定位器名称即复合定位器名称。 名称中不能使用空格或特殊符号。 最大名称长度：14 个字符。</para>
		/// </param>
		/// <param name="InFieldMap">
		/// <para>Field Map</para>
		/// <para>每个参与定位器所使用的输入字段与复合定位器的输入字段之间的映射。</para>
		/// <para>由参与定位器生成的字段和字段内容。 字段映射部分列出了所有的唯一输入字段。 当您展开该部分时，会显示每个参与地址定位器的所有输入字段出现（子字段）的列表。 这些字段是复合地址定位器的输入字段。 如果由于唯一名称是根据参与地址定位器进行填充的，因而产生了重复的字段（如 ZIP 和邮政编码），那么您可以对子字段进行分组，方法是将子字段拖拽到相应的字段，或使用上箭头或下箭头按钮来移动子字段。</para>
		/// </param>
		/// <param name="OutCompositeAddressLocator">
		/// <para>Output Composite Address Locator</para>
		/// <para>将创建的复合地址定位器。 ArcGIS Pro 仅支持保存在文件文件夹中的定位器。</para>
		/// </param>
		...
		/// <para>每个参与定位器的选择条件。 每个参与定位器仅可添加一个选择条件。</para>
		/// <para>构建复合地址定位器时，您可以基于输入地址字段的值来指定所使用的地址定位器。 例如，如果复合地址定位器使用包含某特定城市街道数据的定位器，则可以过滤掉没有此特定城市名称的所有地址或地点。 使用选择条件将会排除不满足特定地址条件的参与地址定位器，从而提高地理编码过程的效率。 例如，如果为某个街道地址定位器指定了选择条件 &quot;City&quot; = &apos;Atlanta&apos;，则只有包含城市名“Atlanta”的地址才会根据此定位器进行地理编码。</para>
		/// <para>要指定选择条件，请在选择条件列中选中要应用此条件的地址定位器的复选框。 可在文本框中输入一个表达式（例如 &quot;City&quot; = &apos;Atlanta&apos;）或单击“更多”按钮打开指定定位器选择条件对话框并定义相应的条件。</para>
		/// </summary>
