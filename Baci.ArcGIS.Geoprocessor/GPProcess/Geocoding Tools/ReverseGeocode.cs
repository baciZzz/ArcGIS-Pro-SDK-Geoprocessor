...
namespace Baci.ArcGIS.Geoprocessor.GeocodingTools
{
	...
	/// <para>反向地理编码</para>
	/// <para>基于要素类中的点位置创建地址。 在反向地理编码过程中，根据使用创建定位器工具创建的定位器的优化距离值来搜索点位置的最近地址、地点或交叉点。</para>
	/// </summary>
	public class ReverseGeocode : AbstractGPProcess
	{
		...
		/// <para>指定将返回的可能匹配类型。 可选择单个值或多个值。 如果选择单个值，则输入要素类型的搜索容差为 500 米。 如果包括多个值，则将应用要素类型等级表中指定的默认搜索距离。 有关反向地理编码的要素类型参数的详细信息，请参阅要素类型。</para>
		/// <para>此参数并非支持所有定位器。</para>
		/// <para>子地址—匹配将限于街道地址（基于代表房屋和建筑物子地址位置的点）。 如果作为服务发布，则此选项需要通过 ArcGIS Pro 2.8 或更高版本及 ArcGIS Enterprise 10.9 或更高版本中创建的定位器。</para>
		...
		/// <para>街道名称—匹配将限于类似于街道地址的地址，但没有门牌号、行政区域和邮政编码（可选）的街道地址，例如 W Olive Ave, Redlands, CA, 92373。</para>
		/// <para>地点—匹配将限于代表居民区的地名。</para>
