...
namespace Baci.ArcGIS.Geoprocessor.GeocodingTools
{
	...
	/// <para>地理编码地址</para>
	/// <para>对地址表进行地理编码。 此过程需要一个存储有要进行地理编码的地址的表和一个地址定位器或复合地址定位器。 此工具根据定位器来匹配地址并将每个输入记录的结果保存在新的点要素类中。 使用 ArcGIS World Geocoding Service 时，此操作可能会消耗配额。</para>
	/// </summary>
	public class GeocodeAddresses : AbstractGPProcess
	{
		...
		/// <para>地址定位器所使用的地址字段与输入地址表中的字段之间的映射。 如果完整地址储存在输入表的一个字段中，例如 303 Peachtree St NE, Atlanta, GA 30308，请选择单个字段。 如果将常规美国地址的输入地址拆分成“地址”、“城市”、“州”和“邮政编码”等多个字段，请选择多个字段。</para>
		...
		/// <para>经过地理编码的输出要素类或 shapefile。</para>
		/// </param>
		...
		/// <para>此参数在 ArcGIS Pro 中始终禁用。 保留它是为了支持 ArcGIS Desktop 向后兼容。</para>
		...
		/// <summary>
		/// <para>Country</para>
		/// <para>此参数可用于支持国家/地区参数的定位器，并将地理编码限制在所选国家/地区。 选择一个国家/地区将在大多数情况下提高地理编码的准确性。 如果将表示输入表中国家/地区的字段映射到输入地址字段中的 Country 字段，则输入表中的国家/地区值将会覆盖国家/地区参数。</para>
		/// <para>这仅限于所选的一个或多个国家/地区。 如果未指定国家/地区，则对定位器的所有受支持国家/地区进行地理编码。</para>
		/// <para>国家/地区不受所有定位器支持。</para>
		/// </summary>
		[ParamType(ParamTypeEnum.optional)]
		[GPMultiValue()]
		[GPCodedValueDomain()]
		...
		/// <para>地址位置—返回地理编码结果的几何，该几何可以表示地址位置，例如屋顶位置、宗地质心或前门。</para>
		/// <para>路径位置—返回地理编码结果的几何，该几何表示靠近街道一侧的位置，可用于车辆配送。 这是默认设置。</para>
		...
		/// <summary>
		/// <para>Category</para>
		/// <para>限制定位器搜索的地点类型，从而消除误报匹配并可能加快搜索过程的速度。 在没有使用类别的情况下，对所有支持的类别进行地理编码。 并非所有位置和国家/地区都支持所有类别值。 通常，该参数可以用于以下几个方面：</para>
		...
		/// <para>仅位置—在地理编码结果中存储 Shape 字段。 输入表参数中的原始字段名称与其原始字段名称一起保留。 使用此选项无法重新匹配地理编码结果。</para>
