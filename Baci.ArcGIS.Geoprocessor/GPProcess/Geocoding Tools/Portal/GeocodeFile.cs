...
namespace Baci.ArcGIS.Geoprocessor.GeocodingTools
{
	...
	public class GeocodeFile : AbstractGPProcess
	{
		...
		/// <para>定位器使用的地址字段会映射到输入地址表中的字段。 如果完整地址储存在输入表的一个字段中，例如 303 Peachtree St NE, Atlanta, GA 30308，请选择单个字段。 如果将常规美国地址的输入划分为 Address、City、State 和 ZIP 等多个字段，请选择多个字段。 如果将完整地址和国家/地区拆分为多个字段，例如，303 Peachtree St NE, Atlanta, GA 30308 和 USA 被拆分为 Address 和 Country，则选择单个字段和国家/地区字段。</para>
		...
		/// <summary>
		/// <para>Country</para>
		/// <para>此参数可用于支持国家/地区参数的定位器，并将地理编码限制在所选国家/地区。 选择一个国家/地区将在大多数情况下提高地理编码的准确性。 当您在地址字段映射中选择单个字段和国家/地区字段，并将输入表中代表国家/地区的字段映射到地址字段映射中的 Country 字段时，输入表中的国家/地区值将覆盖国家/地区参数。</para>
		...
		/// <para>仅位置—如果地理编码结果是要素类，将存储 Shape 字段。 如果结果为 .csv 或 .xls 文件，将存储 Shape X 和 Shape Y 字段。 输入表参数中的原始字段名称与其原始字段名称一起保留。</para>
