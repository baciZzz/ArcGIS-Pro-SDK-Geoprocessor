...
namespace Baci.ArcGIS.Geoprocessor.GeocodingTools
{
	...
	public class GeocodeLocationsFromTable : AbstractGPProcess
	{
		...
		/// <para>The table on the portal that contains the addresses or places to geocode.</para>
		/// </param>
		...
		/// <para>If you do not map an optional input address field used by the locator to a field in the input table of addresses, specify that there is no mapping by using &lt;None&gt; in place of a field name.</para>
		/// </param>
		...
		/// <summary>
		/// <para>Country</para>
		/// <para>The country or countries to search for the geocoded addresses.</para>
		/// <para>This parameter is available for locators that support a country parameter and will limit geocoding to the selected countries. Making a country selection will improve the accuracy of geocoding in most cases. When you select Single Field and Country Field in Address Field Mapping and map a field representing countries in the Input Table parameter to the Country field in Address Field Mapping, the country value from the Input Table parameter will override the Country parameter.</para>
		...
		/// <para>Specifies the preferred output geometry that will be returned for PointAddress matches. The options for this parameter are Routing location, which is the side of street location that can be used for routing and Address location, which is the location that represents the rooftop or parcel centroid for the address. If the preferred location does not exist in the data, the default location will be returned. For geocode results with Addr_type=PointAddress, the x,y attribute values describe the coordinates of the address along the street, while the DisplayX and DisplayY values describe the rooftop, or building centroid, coordinates.</para>
		...
		/// <summary>
		/// <para>Category</para>
		/// <para>Limits the types of places the locator searches, eliminating false positive matches and potentially speeding up the search process. When no category is used, geocoding is performed using all supported categories. Not all category values are supported for all locations and countries. In general, this parameter can be used for the following:</para>
