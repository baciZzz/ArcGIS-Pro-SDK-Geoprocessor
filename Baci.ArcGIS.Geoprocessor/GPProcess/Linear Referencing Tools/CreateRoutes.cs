...
namespace Baci.ArcGIS.Geoprocessor.LinearReferencingTools
{
	...
	/// <para>创建路径</para>
	/// <para>根据现有的线创建路径。如果输入线状要素具有相同的标识符，则将它们将合并以创建单条路径。</para>
	/// </summary>
	public class CreateRoutes : AbstractGPProcess
	{
		...
		/// <para>要素长度—使用输入要素的几何长度累积测量值。这是默认设置。</para>
		/// <para>单个字段的值—使用单个字段中存储的值累积测量值。</para>
		/// <para>两个字段的值—使用“测量始于字段”和“测量止于字段”中存储的值设置测量值。</para>
